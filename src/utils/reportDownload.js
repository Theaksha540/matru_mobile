...
const saveToDownloadsFolder = async ({ content, fileName, mimeType, encoding }) => {
  try {
...
const persistReport = async ({ content, fileName, mimeType, encoding }) => {
  if (Platform.OS !== 'android') {
    // For iOS, save to app directory and share
    await ensureReportsDirectory();
    const localPath = `${REPORTS_DIRECTORY}${fileName}`;
    await FileSystem.writeAsStringAsync(localPath, content, { encoding });
    
    const sharingAvailable = await Sharing.isAvailableAsync();
    if (sharingAvailable) {
      await Sharing.shareAsync(localPath, { mimeType });
    }
...
export const downloadBlobReport = async ({ blob, fileName, mimeType }) => {
...
export const downloadRemoteReport = async ({
  ...
  try {
    ...
    return downloadBlobReport({ blob, fileName, mimeType });
  } catch (error) {
...
export const downloadTextReport = async ({ content, fileName, mimeType, encoding = 'utf8' }) => {
  return persistReport({
    content,
    fileName,
    mimeType,
...
export const resetDownloadPermissions = async () => {
  ...
  Alert.alert(
    'Permissions Reset',
    'Download folder permission has been reset. You will be asked to select it again on your next download.',
    [{ text: 'OK' }]
  );
};
