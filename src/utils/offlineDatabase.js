...
class OfflineDatabase {
  ...
  async encrypt(data) {
    ...
    const hash = await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, jsonString + this.encryptionKey);
  ...
  async saveSecureData(type, id, data) {
    try {
      ...
      await this.runDatabaseOperation(async () => {
        await this.db.runAsync(`INSERT OR REPLACE INTO secure_data (id, type, encrypted_data, hash, updated_at)
           VALUES (?, ?, ?, ?, strftime('%s', 'now'))`, [`${type}_${id}`, type, encryptedData, hash]);
      });
      return true;
    } catch (error) {
      return false;
  ...
  async getSecureData(type, id = null) {
    try {
      return await this.runDatabaseOperation(async () => {
        if (id) {
          const result = await this.db.getFirstAsync('SELECT encrypted_data, hash FROM secure_data WHERE id = ?', [`${type}_${id}`]);
          if (!result) return null;
          return await this.decrypt(result.encrypted_data, result.hash);
        }
        const results = await this.db.getAllAsync('SELECT encrypted_data, hash FROM secure_data WHERE type = ? ORDER BY updated_at DESC', [type]);
        const decrypted = [];
  ...
  async saveECGReports(reports) {
    await this.saveSecureData('ecg_reports', 'all', reports);
  }
  async getECGReports() {
    return (await this.getSecureData('ecg_reports', 'all')) || [];
  }
