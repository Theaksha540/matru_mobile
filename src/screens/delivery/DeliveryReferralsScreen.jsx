...
import { ArrowLeft, UserCheck, RefreshCw, CheckCircle, Clock, Phone, User, Search, AlertCircle, WifiOff, Home } from 'lucide-react-native';
...
const DeliveryReferralsScreen = ({
  ...
  const loadReferrals = async () => {
    try {
      setLoading(true);
      const [data, offlineReferralActions] = await Promise.all([syncService.getDeliveryReferrals(), syncService.getOfflineReferralActions()]);
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
        ...
        y: 1
          ...
          <View style={styles.headerInfo}>
            <Text style={styles.headerTitle}>
              {initialFilter === 're_referred' ? 'Re-referred Cases' : initialFilter === 'pending' ? 'Pending Referrals' : t('deliveryReferrals') || 'Delivery Referrals'}
            </Text>
            <Text style={styles.headerSubtitle}>
              {initialFilter === 're_referred' ? 'ପୁନଃ ରେଫର କେସ' : initialFilter === 'pending' ? 'ବାକି ରେଫରାଲ' : 'ଡେଲିଭରି ରେଫରାଲ'}
            </Text>
          </View>
        ...
        <View style={styles.filterContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScrollContent}>
            {filters.map(filterItem => <TouchableOpacity key={filterItem.key} style={[styles.filterTab, filter === filterItem.key && styles.activeFilterTab]} onPress={() => handleFilterChange(filterItem.key)}>
                <Text style={[styles.filterTabText, filter === filterItem.key && styles.activeFilterTabText]}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#8B4513']} tintColor="#8B4513" />}>
          ...
          <View style={styles.searchContainer}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search by patient name, mobile, or DP..." placeholderTextColor="#9ca3af" value={searchQuery} onChangeText={handleSearchChange} returnKeyType="search" autoCorrect={false} autoCapitalize="none" />
            {searchQuery.length > 0 && <TouchableOpacity onPress={() => handleSearchChange('')} style={styles.clearSearchButton} activeOpacity={0.7}>
          ...
          {loading ? <View style={styles.emptyStateContainer}>
              <ActivityIndicator size="large" color="#D2691E" />
              <Text style={styles.loadingText}>{t('loadingReferrals') || 'Loading referrals...'}</Text>
            </View> : paginatedReferrals.length === 0 ? <View style={styles.emptyStateContainer}>
            ...
            return <TouchableOpacity key={referral.id} style={styles.referralCard} onPress={() => {
              navigation.navigate('ReferralDetail', {
                referralId: referral.id
                  ...
                  {referral.referral_chain_summary && <Text style={styles.observationNotes} numberOfLines={2}>
                      Chain: {referral.referral_chain_summary}
                    </Text>}

                  {/* Re-referral specific information */}
                  {referral.status === 're_referred' && referral.re_refer_reason && <View style={styles.reReferInfo}>
                  ...
                  {referral.updated_at && referral.updated_at !== referral.created_at && <View style={styles.additionalDateInfo}>
                  ...
                  {referral.observation_notes && <Text style={styles.observationNotes} numberOfLines={2}>
                      {referral.observation_notes}
                    </Text>}

                  {/* Action Buttons */}
                  {(userInfo?.role === 'dp' || userInfo?.role === 'sub_centre' && referral.status === 'accepted') && <View style={styles.actionButtons}>
                      {referral.status === 'pending' && <>
                          <TouchableOpacity style={[styles.actionButton, styles.acceptButton]} onPress={() => handleAcceptReferral(referral.id)}>
                            <UserCheck size={16} color="white" />
                            <Text style={styles.acceptButtonText}>Accept</Text>
                          </TouchableOpacity>
                          
                          <TouchableOpacity style={[styles.actionButton, styles.reReferButton]} onPress={() => handleReRefer(referral.id)}>
                      ...
                      {referral.status === 'accepted' && <>
                          <TouchableOpacity style={[styles.actionButton, styles.outcomeButton]} onPress={() => handleRecordOutcome(referral.id)}>
                            <CheckCircle size={16} color="#059669" />
                            <Text style={styles.outcomeButtonText}>Record Outcome</Text>
                          </TouchableOpacity>

                          {userInfo?.role === 'dp' && <TouchableOpacity style={[styles.actionButton, styles.reReferButton]} onPress={() => handleReRefer(referral.id)}>
                              <RefreshCw size={16} color="#dc2626" />
                              <Text style={styles.reReferButtonText}>Re-refer</Text>
                            </TouchableOpacity>}
                        </>}
            ...
            <PaginationControls currentPage={currentPage} totalPages={totalPages} totalCount={totalCount} startIndex={startIndex} endIndex={endIndex} onPreviousPage={previousPage} onNextPage={nextPage} hasPreviousPage={hasPreviousPage} hasNextPage={hasNextPage} loading={loading} />
            </>}
        
        {/* Info Banner */}
        {paginatedReferrals.length > 0 && <LinearGradient colors={['#fad5a5', '#fff4e6']} start={{
          x: 0,
          y: 0
        }} end={{
          x: 1,
