...
import { ArrowLeft, UserCheck, RefreshCw, Clock, Phone, User, AlertCircle, MapPin, Calendar, FileText, Heart, Baby, Activity, Wifi, WifiOff, CheckCircle, Home } from 'lucide-react-native';
...
const ReferralDetailScreen = ({
  ...
  const flattenReferralData = data => {
    ...
    const gestationalAge = patient?.gestational_age || referralCore?.gestational_age || data?.gestational_age || calculateGestationalAge({
      lmp_date: lmpDate,
      edd_date: eddDate,
    ...
    return {
      ...referralCore,
      pregnant_woman_name: patient?.full_name || data?.pregnant_woman_name || data?.patient_name || data?.full_name,
      patient_name: patient?.full_name || data?.patient_name || data?.pregnant_woman_name || data?.full_name,
      mobile_number: patient?.mobile_number || data?.mobile_number || data?.patient_mobile || data?.pregnant_woman_mobile,
      phone: patient?.mobile_number || data?.mobile_number || data?.patient_mobile || data?.pregnant_woman_mobile,
      ...
      treatment_given: referralCore?.treatment_given || data?.treatment_given,
      re_refer_reason: referralCore?.re_refer_reason || data?.re_refer_reason,
      previous_referral_id: referralCore?.previous_referral_id || data?.previous_referral_id,
      _original: data
    };
  };
  const findReferralSummaryById = async targetReferralId => {
    try {
      ...
      const list = Array.isArray(referrals) ? referrals : Array.isArray(referrals?.items) ? referrals.items : [];
      ...
      const matchedReferral = list.find(item => {
        const stringCandidateIds = [item?.id, item?.referral_id, item?.delivery_referral_id, item?.reference_id, item?.previous_referral_id].filter(value => value !== undefined && value !== null).map(value => String(value));
        if (stringCandidateIds.includes(targetReferralIdString)) {
          return true;
        }
        const candidateIds = [item?.id, item?.referral_id, item?.delivery_referral_id, item?.reference_id, item?.previous_referral_id].map(value => Number(value)).filter(value => Number.isFinite(value));
        ...
        return chain.some(step => {
          const chainStringIds = [step?.referral_id, step?.previous_referral_id, step?.reference_id].filter(value => value !== undefined && value !== null).map(value => String(value));
          if (chainStringIds.includes(targetReferralIdString)) {
            return true;
          }
          const chainIds = [step?.referral_id, step?.previous_referral_id, step?.reference_id].map(value => Number(value)).filter(value => Number.isFinite(value));
    ...
    try {
      ...
      try {
        ...
        if (status === 403 || status === 404) {
          const fallbackReferral = route?.params?.referralData && Number(route.params.referralData?.id) === Number(referralId) ? route.params.referralData : await findReferralSummaryById(referralId);
          if (fallbackReferral) {
  ...
  const loadUSGCentreNames = async (appointments = []) => {
    try {
      const centreIds = [...new Set((Array.isArray(appointments) ? appointments : []).map(item => item?.usg_centre_id).filter(Boolean))];
  ...
  const handleAcceptReferral = async () => {
    Alert.alert(t('acceptReferral'), t('acceptReferralConfirmation'), [{
      text: t('cancel'),
      style: 'cancel'
    }, {
  ...
  const handleReRefer = () => {
    navigation.navigate('ReReferScreen', {
      referralId
    });
  };
  const handleRecordOutcome = () => {
    navigation.navigate('RecordOutcomeScreen', {
      referralId
    });
  };
  ...
  const getReReferAttachmentUrls = chainStep => {
    const multipleUrls = Array.isArray(chainStep?.re_refer_attachment_urls) ? chainStep.re_refer_attachment_urls.filter(Boolean) : [];
    if (multipleUrls.length > 0) {
      return multipleUrls;
    }
    if (chainStep?.re_refer_attachment_url) {
      return [chainStep.re_refer_attachment_url];
    }
    return [];
  };
  const getCachePath = (type, appointmentId, extension) => `${FileSystem.cacheDirectory}${type}_${appointmentId}.${extension}`;
  ...
  const getUSGCentreDisplayName = usg => {
    return usg?.usg_centre_name || usgCentreMap[usg?.usg_centre_id] || (usg?.usg_centre_id ? `USG Centre ${usg.usg_centre_id}` : null);
  };
  const getECGResultDisplay = result => {
    if (!result) return notSpecifiedText;
    return String(result).replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase());
  };
  ...
  const formatDeliveryType = deliveryType => {
    ...
    return String(deliveryType).replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase());
  };
  ...
  const getGestationalAgeDisplay = referralData => {
    ...
    if (referralData.gestational_age !== undefined && referralData.gestational_age !== null && String(referralData.gestational_age).trim() !== '') {
      ...
      if (compactMatch) {
        ...
        return daysValue ? `${weeksValue} ${t('weeks') || 'weeks'} ${daysValue} ${t('days') || 'days'}` : `${weeksValue} ${t('weeks') || 'weeks'}`;
      }
  ...
  const buildReferralFlow = referralData => {
    const chain = Array.isArray(referralData?.referral_chain) ? referralData.referral_chain.filter(Boolean) : [];
    return chain.map((chainItem, index) => {
      const previousStep = chain[index - 1];
      const fromLabel = getFirstMeaningfulValue(chainItem?.referred_by, previousStep?.dp_name, previousStep?.referred_to, index === 0 ? referralData?.sub_centre_name : null, index === 0 ? t('initialReferralSource') : t('previousDeliveryPoint'));
      const toLabel = getFirstMeaningfulValue(chainItem?.referred_to, chainItem?.dp_name, index === chain.length - 1 ? referralData?.dp_name : null, notSpecifiedText);
      return {
        ...chainItem,
        fromLabel,
        toLabel
  ...
  const currentReferralStatus = referral.status || 'pending';
  const canDpManageReferral = userInfo?.role === 'dp';
  const canSubCentreRecordOutcome = userInfo?.role === 'sub_centre' && currentReferralStatus === 'accepted';
  const showRoleRestrictionMessage = !canDpManageReferral && !canSubCentreRecordOutcome && (currentReferralStatus === 'pending' || currentReferralStatus === 'accepted');
  return <SafeAreaProvider>
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
        ...
        {/* Content */}
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#8B4513']} tintColor="#8B4513" />}>
          ...
          y: 1
              ...
              <View style={styles.offlineInfoContent}>
                ...
                {offlineReferralActionCount > 0 && <Text style={styles.offlineInfoQueued}>
                    {t('offlineReferralActionsWaiting', {
                count: offlineReferralActionCount
              })}
                  </Text>}
              </View>
            </LinearGradient>}
          ...
          {/* ANC Visits Card */}
          {referral.anc_visits && referral.anc_visits.length > 0 && <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Activity size={24} color="#D2691E" />
                <Text style={styles.cardTitle}>{t('ancVisitsCount', {
                count: referral.anc_visits.length
              })}</Text>
              </View>
              <View style={styles.cardContent}>
                {referral.anc_visits.map((visit, index) => <View key={index} style={styles.visitItem}>
                    ...
                    <View style={styles.visitDetails}>
                      ...
                      {visit.is_emergency !== undefined && visit.is_emergency && <View style={styles.emergencyBadge}>
                          <AlertCircle size={14} color="#dc2626" />
                          <Text style={styles.emergencyBadgeText}>{t('emergencyVisit')}</Text>
                        </View>}
                      {visit.doctor_notes && visit.doctor_notes.trim() !== '' && <View style={styles.notesSection}>
          ...
          {referral.usg_appointments && referral.usg_appointments.length > 0 && <View style={styles.card}>
              <View style={styles.cardHeader}>
                <FileText size={24} color="#D2691E" />
                <Text style={styles.cardTitle}>{t('usgAppointmentsCount', {
                count: referral.usg_appointments.length
              })}</Text>
              </View>
              <View style={styles.cardContent}>
                {[...referral.usg_appointments].sort((first, second) => {
                const firstDate = new Date(first?.completed_date || first?.scheduled_date || first?.created_at || 0).getTime();
                const secondDate = new Date(second?.completed_date || second?.scheduled_date || second?.created_at || 0).getTime();
                return secondDate - firstDate;
              }).map((usg, index) => <View key={index} style={styles.visitItem}>
                    <View style={styles.visitHeader}>
                      <Text style={styles.visitNumber}>{t('usgLabel', {
                    count: index + 1
                  })}</Text>
                      <View style={styles.usgHeaderRight}>
                        {usg.status && <View style={[styles.usgStatusBadge, {
                    backgroundColor: getStatusBgColor(usg.status)
                  }]}>
                            <Text style={[styles.usgStatusText, {
                      color: getStatusColor(usg.status)
                    }]}>
                    ...
                    <View style={styles.visitDetails}>
                      ...
                      {usg.created_at && <View style={styles.visitDetailRow}>
                      ...
                      {usg.reschedule_count !== undefined && <View style={styles.visitDetailRow}>
                          <Text style={styles.visitDetailLabel}>{t('rescheduleCount')}:</Text>
                          <Text style={styles.visitDetailValue}>{usg.reschedule_count}</Text>
                        </View>}
                      {usg.is_high_risk !== undefined && <View style={styles.visitDetailRow}>
                      ...
                      {usg.findings && usg.findings.trim() !== '' && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('findingsLabel')}:</Text>
                          <Text style={styles.visitNotes}>{usg.findings}</Text>
                        </View>}
                      {usg.usg_findings && usg.usg_findings.trim() !== '' && usg.usg_findings !== usg.findings && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('usgFindingsLabel')}:</Text>
                          <Text style={styles.visitNotes}>{usg.usg_findings}</Text>
                        </View>}
                      {usg.abnormal_findings && usg.abnormal_findings.trim() !== '' && <View style={styles.notesSection}>
                      ...
                      {usg.additional_notes && usg.additional_notes.trim() !== '' && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('additionalNotesLabel')}:</Text>
                          <Text style={styles.visitNotes}>{usg.additional_notes}</Text>
                        </View>}
                      {usg.reschedule_reason && usg.reschedule_reason.trim() !== '' && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('rescheduleReasonLabel')}:</Text>
                          <Text style={styles.visitNotes}>{usg.reschedule_reason}</Text>
                        </View>}
                      {getPrescriptionUrls(usg).length > 0 && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('prescriptionFileLabel')}:</Text>
                          {getPrescriptionUrls(usg).map((fileUrl, index) => <TouchableOpacity key={`${usg.id}_prescription_${index}`} style={styles.fileLinkButton} onPress={() => openFile(fileUrl, 'prescription', `${usg.id}_${index}`)}>
                              <Text style={styles.fileLinkText}>{`${t('viewOrDownloadPrescription')} ${index + 1}`}</Text>
                            </TouchableOpacity>)}
                        </View>}
                      {usg.report_file_url && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('reportFileLabel')}:</Text>
                          <TouchableOpacity style={styles.fileLinkButton} onPress={() => openFile(usg.report_file_url, 'report', usg.id)}>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              backgroundColor: getStatusBgColor(referral.status || 'pending')
            }]}>
                <Text style={[styles.headerStatusText, {
                color: getStatusColor(referral.status || 'pending')
              }]}>
            ...
            <View style={styles.cardContent}>
              ...
              {latestReferralFlow && <View style={styles.routeSection}>
                  ...
                  {referralFlow.length > 1 && <View style={styles.routeHistoryList}>
                      {referralFlow.map((step, index) => <View key={`${step.referral_id || index}_${index}`} style={styles.routeHistoryItem}>
              ...
              <View style={styles.timelineSection}>
                ...
                <View style={styles.timelineItems}>
                  ...
                  {referral.accepted_at && <View style={styles.timelineItem}>
                      <View style={styles.timelineIcon}>
              ...
              {referral.previous_referral_id && <View style={styles.previousReferralSection}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardContent}>
              ...
              {referral.treatment_given && <View style={styles.infoColumn}>
                  <Text style={styles.infoLabel}>Treatment Given:</Text>
                  <View style={styles.notesContainer}>
                    <Text style={styles.notesText}>
              ...
              {referral.re_refer_reason && <View style={styles.infoColumn}>
              ...
              <View style={styles.infoColumn}>
                ...
                <View style={styles.notesContainer}>
                  ...
                  {referral.risk_factors && <Text style={styles.riskFactorsText}>
          ...
          {/* Referral Chain Card */}
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <Text style={styles.cardTitle}>{t('referralChainCount', {
                count: referral.referral_chain.length
              })}</Text>
              </View>
              <View style={styles.cardContent}>
                ...
                {referral.referral_chain.map((chain, index) => <View key={index} style={styles.chainItemExpanded}>
                    <View style={styles.chainHeaderExpanded}>
                      ...
                      <View style={[styles.chainStatusBadge, {
                  backgroundColor: getStatusBgColor(chain.status)
                }]}>
                        <Text style={[styles.chainStatusBadgeText, {
                    ...
                    <View style={styles.chainDetailsExpanded}>
                      ...
                      {/* Medical Information */}
                      {(chain.observation_notes || chain.treatment_given || chain.re_refer_reason || getReReferAttachmentUrls(chain).length > 0) && <View style={styles.chainSection}>
                          <Text style={styles.chainSectionTitle}>{t('medicalInformation')}</Text>
                          {chain.observation_notes && <View style={styles.chainNotesContainer}>
                              <Text style={styles.chainNotesLabel}>{t('observationNotes')}:</Text>
                              <Text style={styles.chainNotesText}>{chain.observation_notes}</Text>
                            </View>}
                          {chain.treatment_given && <View style={styles.chainNotesContainer}>
                              <Text style={styles.chainNotesLabel}>Treatment Given:</Text>
                              <Text style={styles.chainNotesText}>{chain.treatment_given}</Text>
                            </View>}
                          {chain.re_refer_reason && <View style={styles.chainNotesContainer}>
                          ...
                          {getReReferAttachmentUrls(chain).length > 0 && <View style={styles.chainNotesContainer}>
                              <Text style={styles.chainNotesLabel}>Attachments:</Text>
                              {getReReferAttachmentUrls(chain).map((fileUrl, attachmentIndex) => <TouchableOpacity key={`${chain.referral_id || index}_attachment_${attachmentIndex}`} style={styles.fileLinkButton} onPress={() => openFile(fileUrl, 're_refer_attachment', `${chain.referral_id || index}_${attachmentIndex}`)}>
                                  <Text style={styles.fileLinkText}>{`View Attachment ${attachmentIndex + 1}`}</Text>
                                </TouchableOpacity>)}
                            </View>}
                        </View>}
                    </View>
           ...
           {/* ECG Reports Card */}
          {referral.ecg_reports && referral.ecg_reports.length > 0 && <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Heart size={24} color="#D2691E" />
                <Text style={styles.cardTitle}>{t('ecgReportsCount', {
                count: referral.ecg_reports.length
              })}</Text>
              </View>
              <View style={styles.cardContent}>
                {[...referral.ecg_reports].sort((first, second) => {
                const firstDate = new Date(first?.ecg_date || first?.created_at || 0).getTime();
                const secondDate = new Date(second?.ecg_date || second?.created_at || 0).getTime();
                return secondDate - firstDate;
              }).map((report, index) => <View key={report?.id || index} style={styles.visitItem}>
                    <View style={styles.visitHeader}>
                      <Text style={styles.visitNumber}>{t('ecgEntryLabel', {
                      count: index + 1
                    })}</Text>
                      <Text style={styles.visitDate}>{formatDate(report.ecg_date)}</Text>
                    </View>
                    <View style={styles.visitDetails}>
                      <View style={styles.visitDetailRow}>
                        <Text style={styles.visitDetailLabel}>{t('result')}:</Text>
                        <Text style={styles.visitDetailValue}>{getECGResultDisplay(report.result)}</Text>
                      </View>
                      {(report.recorded_by_name || report.dp_name) && <View style={styles.visitDetailRow}>
                          <Text style={styles.visitDetailLabel}>{t('recordedBy')}:</Text>
                          <Text style={styles.visitDetailValue}>{report.recorded_by_name || report.dp_name}</Text>
                        </View>}
                      {report.dp_name && <View style={styles.visitDetailRow}>
                          <Text style={styles.visitDetailLabel}>{t('deliveryPoint')}:</Text>
                          <Text style={styles.visitDetailValue}>{report.dp_name}</Text>
                        </View>}
                      {report.created_at && <View style={styles.visitDetailRow}>
                          <Text style={styles.visitDetailLabel}>{t('createdAtLabel')}:</Text>
                          <Text style={styles.visitDetailValue}>{formatDateTime(report.created_at)}</Text>
                        </View>}
                      {report.notes && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('notes')}:</Text>
                          <Text style={styles.visitNotes}>{report.notes}</Text>
                        </View>}
                      {report.report_file_url && <View style={styles.notesSection}>
                          <Text style={styles.notesLabel}>{t('reportFileLabel')}:</Text>
                          <TouchableOpacity style={styles.fileLinkButton} onPress={() => openFile(report.report_file_url, 'ecg_report', report.id)}>
                            <Text style={styles.fileLinkText}>{t('viewOrDownloadEcgReport')}</Text>
                          </TouchableOpacity>
                        </View>}
                    </View>
                  </View>)}
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardContent}>
              {!referral.outcome && <Text style={styles.infoValue}>{t('outcomeNotRecordedYet')}</Text>}
              {outcome.delivery_type && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('deliveryType')}:</Text>
                  <Text style={styles.infoValue}>{formatDeliveryType(outcome.delivery_type)}</Text>
                </View>}
              {outcome.delivery_date && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('deliveryDate')}:</Text>
                  <Text style={styles.infoValue}>{formatDateTime(outcome.delivery_date)}</Text>
                </View>}
              {babies.length > 0 && <View style={styles.infoColumn}>
                  <Text style={styles.infoLabel}>{t('babies')}:</Text>
                  <View style={styles.notesContainer}>
                    {babies.map((baby, index) => <View key={`${baby.gender || 'baby'}_${index}`} style={[styles.babyOutcomeRow, index !== babies.length - 1 && styles.babyOutcomeRowBorder]}>
                        <Text style={styles.babyOutcomeTitle}>{t('babyLabel', {
                      count: index + 1
                    })}</Text>
              ...
              {outcome.delivery_time && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('deliveryTime')}:</Text>
                  <Text style={styles.infoValue}>{outcome.delivery_time}</Text>
                </View>}
              {outcome.baby_weight && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('babyWeight')}:</Text>
                  <Text style={styles.infoValue}>{outcome.baby_weight} kg</Text>
                </View>}
              {outcome.baby_gender && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('babyGender')}:</Text>
                  <Text style={styles.infoValue}>{formatOutcomeValue(outcome.baby_gender)}</Text>
                </View>}
              {outcome.mother_condition && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('motherCondition')}:</Text>
                  <Text style={styles.infoValue}>{formatOutcomeValue(outcome.mother_condition)}</Text>
                </View>}
              {outcome.baby_condition && <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>{t('babyCondition')}:</Text>
                  <Text style={styles.infoValue}>{formatOutcomeValue(outcome.baby_condition)}</Text>
                </View>}
              {outcome.complications && <View style={styles.infoColumn}>
              ...
              {outcome.remarks && <View style={styles.infoColumn}>
          ...
          {/* Action Buttons */}
          {canDpManageReferral && <View style={styles.actionSection}>
              {currentReferralStatus === 'pending' && <>
                  <TouchableOpacity style={styles.acceptButton} onPress={handleAcceptReferral}>
                    <UserCheck size={20} color="white" />
                    <Text style={styles.acceptButtonText}>{t('acceptReferral')}</Text>
                  </TouchableOpacity>
                  
                  <TouchableOpacity style={styles.reReferButton} onPress={handleReRefer}>
              ...
              {currentReferralStatus === 'accepted' && <>
                  <TouchableOpacity style={styles.outcomeButton} onPress={handleRecordOutcome}>
          ...
          {canSubCentreRecordOutcome && <View style={styles.actionSection}>
              <TouchableOpacity style={styles.outcomeButton} onPress={handleRecordOutcome}>
                <CheckCircle size={20} color="#059669" />
                <Text style={styles.outcomeButtonText}>{t('recordOutcome')}</Text>
              </TouchableOpacity>
          ...
          {showRoleRestrictionMessage && <View style={styles.infoMessageContainer}>
