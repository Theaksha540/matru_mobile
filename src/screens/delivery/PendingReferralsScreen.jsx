...
import { ArrowLeft, UserCheck, RefreshCw, Clock, Phone, User, AlertCircle, Wifi, WifiOff, Search, Home } from 'lucide-react-native';
...
const PendingReferralsScreen = ({
  ...
  const loadPendingReferrals = async () => {
    try {
      setLoading(true);
      const [data, offlineReferralActions] = await Promise.all([syncService.getDeliveryReferrals('pending'), syncService.getOfflineReferralActions()]);
      const filteredData = Array.isArray(data) ? data.filter(referral => referral?.status === 'pending') : [];
  ...
  const handleAcceptReferral = async referralId => {
    Alert.alert('Accept Referral', 'Are you sure you want to accept this delivery referral?', [{
      text: 'Cancel',
      style: 'cancel'
    }, {
      text: 'Accept',
  ...
  const filteredReferrals = referrals.filter(referral => {
    ...
    return [referral.pregnant_woman_name, referral.mobile_number, referral.dp_name, referral.sub_centre_name, referral.observation_notes, referral.referral_chain_summary].filter(Boolean).some(value => String(value).toLowerCase().includes(normalizedQuery));
  });
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#D2691E']} tintColor="#D2691E" />}>
          ...
          <View style={styles.searchContainer}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search by patient name, mobile, or DP name" value={searchQuery} onChangeText={handleSearchChange} placeholderTextColor="#999" />
          ...
          {loading ? <View style={styles.emptyStateContainer}>
            ...
            </View> : filteredReferrals.length === 0 ? <View style={styles.emptyStateContainer}>
              {searchQuery.trim() !== '' ? <>
              ...
              {/* Priority Notice */}
              <LinearGradient colors={['#fef3c7', '#fff4e6']} start={{
            x: 0,
            y: 0
          }} end={{
            x: 1,
            y: 1
              ...
              {/* Referral Cards */}
              {paginatedReferrals.map(referral => <TouchableOpacity key={referral.id} style={styles.referralCard} onPress={() => handleViewDetails(referral.id)} activeOpacity={0.7}>
                  {/* Card Header */}
                  ...
                  <View style={styles.actionButtons}>
                    <TouchableOpacity style={[styles.actionButton, styles.acceptButton]} onPress={e => {
                e.stopPropagation();
                handleAcceptReferral(referral.id);
                    ...
                    <TouchableOpacity style={[styles.actionButton, styles.reReferButton]} onPress={e => {
                e.stopPropagation();
