...
import { ArrowLeft, FileSpreadsheet, Upload, ClipboardList, CheckCircle, Download } from 'lucide-react-native';
...
const BulkUploadScreen = ({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      Alert.alert(t('success'), `${t('bulkUploadCompletedSuccessfully')}\n\n${t('processed')}: ${result.processed || 'N/A'} ${t('records')}`, [{
        text: 'OK',
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.mainContent}>
            <View style={styles.contentContainer}>
              ...
              <TouchableOpacity style={styles.downloadButton} onPress={downloadTemplate} activeOpacity={0.7}>
                <Download size={16} color="#8B4513" />
                <Text style={styles.downloadButtonText}>Download Template</Text>
              </TouchableOpacity>

              {/* Upload Dropzone */}
                <TouchableOpacity style={styles.uploadDropzone} onPress={handleFilePick} activeOpacity={0.7}>
                ...
                {selectedFile && <View style={styles.selectedFile}>
                    <View style={styles.fileInfo}>
                      <CheckCircle size={16} color="#22c55e" />
                      <Text style={styles.selectedFileName}>{selectedFile.name}</Text>
                      <TouchableOpacity style={styles.removeFileButton} onPress={() => setSelectedFile(null)} activeOpacity={0.7}>
                    ...
                    <TouchableOpacity style={[styles.uploadButton, uploading && styles.disabledButton]} onPress={handleUpload} disabled={uploading}>
                      {uploading ? <ActivityIndicator size={16} color="white" /> : <Upload size={16} color="white" />}
              ...
              <View style={styles.requirementsCard}>
                ...
                <View style={styles.requirementsList}>
                  {templateInfo ? <>
                      ...
                      {templateInfo.required_columns.map((field, index) => <Text key={index} style={styles.requirementItem}>• {field}</Text>)}
                      <Text style={styles.sectionSubtitle}>All Available Fields:</Text>
                      {templateInfo.template_columns.map((field, index) => <Text key={index} style={styles.requirementItem}>• {field}</Text>)}
