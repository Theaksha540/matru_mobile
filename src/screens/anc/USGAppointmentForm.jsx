...
const USGAppointmentForm = ({ navigation, route }) => {
  const { t } = useTranslation();
  const { patientId, patientName, appointmentToReschedule } = route?.params || {};
    ...
    try {
      ...
      if (patientId) {
        ...
        if (data) {
          ...
          console.log('No patient data found, using route params');
          // Fallback to route params if available
          if (patientName) {
            setPatient({ full_name: patientName, id: patientId });
          }
        }
      }
    } catch (error) {
  ...
  const handleScheduleAppointment = async () => {
    ...
    if (!prescriptionFile && !isReschedule) {
    ...
    try {
      ...
      if (prescriptionFile) {
        appointmentData.prescription_file = {
          ...
          type: prescriptionFile.mimeType || 'image/jpeg',
      ...
      if (isOnline) {
        ...
        Alert.alert(
          t('success'),
          t('usgAppointmentBookedSuccessfully'),
        ...
        Alert.alert(
          t('success'),
          'USG appointment scheduled offline - will sync when online',
          [{ text: 'OK', onPress: () => navigation.goBack() }]
        );
      }
    } catch (error) {
          ...
          <View style={styles.card}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.pickerContainer}>
                ...
                >
                  ...
                  {usgCentres.map((centre) => (
                    <Picker.Item key={centre.id} label={centre.name} value={centre.id} />
                  ))}
                </Picker>
              </View>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TouchableOpacity style={styles.dateInput} onPress={() => setShowTimePicker(true)}>
                <Text style={styles.dateText}>{appointmentTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</Text>
              </TouchableOpacity>
              {showTimePicker && (
                <DateTimePicker
            ...
            <View style={styles.inputGroup}>
              ...
              <TouchableOpacity style={styles.uploadBox} onPress={handlePickFile}>
                ...
                <Text style={styles.uploadText}>
                  {prescriptionFile ? prescriptionFile.name : t('clickToUploadPrescription')}
                </Text>
            ...
            {/* Book Button */}
            <TouchableOpacity
              style={[styles.bookButton, (!selectedCentre || !prescriptionFile || loading || !isPatientApproved(patient)) && styles.bookButtonDisabled]}
              onPress={handleScheduleAppointment}
              disabled={!selectedCentre || !prescriptionFile || loading || !isPatientApproved(patient)}
            >
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: '#D2691E', padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12 },
  ...
  detailItem: { width: '48%', fontSize: 13, color: '#6b7280' },
  fullWidth: { width: '100%' },
  detailLabel: { fontWeight: '600', color: '#1f2937' },
  detailValue: { color: '#6b7280' },
  approvedText: { color: '#166534', fontWeight: '600' },
  notApprovedText: { color: '#991b1b', fontWeight: '600' },
  inputGroup: { marginBottom: 16 },
  inputLabel: { fontSize: 13, fontWeight: '500', color: '#374151', marginBottom: 6 },
  pickerContainer: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, backgroundColor: 'white' },
  picker: { height: 50 },
  dateInput: { padding: 12, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, backgroundColor: 'white' },
  dateText: { fontSize: 14, color: '#1f2937' },
  uploadBox: { borderWidth: 2, borderColor: '#e5e7eb', borderStyle: 'dashed', borderRadius: 8, padding: 20, alignItems: 'center', backgroundColor: '#f9fafb' },
  uploadText: { fontSize: 14, color: '#6b7280', marginTop: 8 },
  uploadHint: { fontSize: 12, color: '#9ca3af', marginTop: 4 },
  emergencyBox: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, backgroundColor: '#fef3c7', borderWidth: 1, borderColor: '#fbbf24', borderRadius: 8, marginBottom: 16 },
  checkbox: { width: 18, height: 18, borderWidth: 2, borderColor: '#92400e', borderRadius: 4, alignItems: 'center', justifyContent: 'center' },
  checkboxChecked: { backgroundColor: '#92400e' },
  checkmark: { color: 'white', fontSize: 12, fontWeight: '600' },
  emergencyText: { flex: 1 },
  emergencyTitle: { fontSize: 14, fontWeight: '600', color: '#92400e' },
  emergencySubtitle: { fontSize: 12, color: '#92400e', marginTop: 2 },
  bookButton: { width: '100%', padding: 14, backgroundColor: '#D2691E', borderRadius: 8, alignItems: 'center' },
  bookButtonDisabled: { backgroundColor: '#d1d5db' },
  bookButtonText: { color: 'white', fontSize: 15, fontWeight: '600' },
  uploadModalOverlay: {
    flex: 1,
