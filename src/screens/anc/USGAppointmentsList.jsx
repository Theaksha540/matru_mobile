...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, RefreshControl, Alert, ActivityIndicator, TextInput } from 'react-native';
...
const USGAppointmentsList = ({
    ...
    try {
      ...
      (Array.isArray(centres) ? centres : []).forEach(centre => {
        if (centre?.id !== undefined && centre?.id !== null) {
          centresLookup[String(centre.id)] = centre.name || centre.centre_name || centre.usg_centre_name || `USG Centre ${centre.id}`;
        }
      });
  ...
  const loadAppointments = async () => {
    try {
      ...
      const patientIds = [...new Set(appointments.map(apt => apt.pregnant_woman_id).filter(id => id !== null && id !== undefined))];
  ...
  const filterAppointments = () => {
    ...
    filtered.sort((first, second) => {
      const firstDate = new Date(first?.created_at || first?.scheduled_date || 0).getTime();
      const secondDate = new Date(second?.created_at || second?.scheduled_date || 0).getTime();
  ...
  const getPatientDisplayName = appointment => {
    return patients[appointment?.pregnant_woman_id]?.full_name || `Patient ${appointment?.pregnant_woman_id ?? 'N/A'}`;
  };
  const renderAppointment = appointment => <TouchableOpacity key={appointment.id} style={styles.appointmentCard} onPress={() => navigation.navigate('AppointmentDetail', {
    appointmentId: appointment.id
  })} activeOpacity={0.7}>
      <View style={styles.appointmentHeader}>
        <View style={styles.appointmentInfo}>
          <Text style={styles.appointmentId}>{getPatientDisplayName(appointment)}</Text>
      ...
      <View style={styles.appointmentDetails}>
        ...
        {appointment.reschedule_count > 0 && <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>{t('reschedules')}:</Text>
            <Text style={styles.detailValue}>{appointment.reschedule_count}</Text>
          </View>}

        {appointment.completed_date && appointment.status === 'completed' && <View style={styles.detailRow}>
  ...
  return <SafeAreaView style={styles.container} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          {!isOnline && <Text style={styles.offlineIndicator}>● {t('offlineMode')}</Text>}
        </View>
      ...
      <View style={styles.filterContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {['all', 'scheduled', 'completed', 'cancelled', 'rescheduled'].map(status => <TouchableOpacity key={status} style={[styles.filterButton, selectedStatus === status && styles.activeFilter]} onPress={() => setSelectedStatus(status)}>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchInputWrapper}>
          ...
          <TextInput style={styles.searchInput} placeholder={t('searchByNameMobileAge')} value={searchQuery} onChangeText={setSearchQuery} placeholderTextColor="#9ca3af" />
        </View>
      </View>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {loading ? <View style={styles.loadingContainer}>
          ...
          </View> : paginatedAppointments.length > 0 ? <>
            {paginatedAppointments.map(renderAppointment)}
            <PaginationControls currentPage={currentPage} totalPages={totalPages} totalCount={filteredAppointments.length} startIndex={startIndex} endIndex={endIndex} onPreviousPage={previousPage} onNextPage={nextPage} hasPreviousPage={hasPreviousPage} hasNextPage={hasNextPage} loading={loading} />
          </> : <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>
              {searchQuery.trim() ? t('noMatchingAppointments') : isOnline ? t('noAppointmentsFound') : t('noCachedAppointments')}
            </Text>
            {!isOnline && !searchQuery.trim() && <Text style={styles.offlineText}>
