...
const ANCUpdateForm = ({
  ...
  const verifyAccess = async () => {
    try {
      ...
      if (userInfo?.role !== 'sub_centre') {
        Alert.alert(t('error'), t('onlySubCentreAnc'), [{
          text: 'OK',
      ...
      Alert.alert(t('error'), t('verifyAncPermissions'), [{
        text: 'OK',
        onPress: () => navigation.goBack()
      }]);
    }
  ...
  const loadPastVisits = async () => {
    try {
      const [syncedVisits, offlineVisits] = await Promise.all([syncService.getANCVisits(patient.id), syncService.getOfflineANCVisits(patient.id)]);
      const allVisits = [...(syncedVisits || []), ...(offlineVisits || [])].filter(Boolean).reduce((visits, visit) => {
  ...
  const handleDateChange = (event, date) => {
    ...
    if (date) {
      ...
      if (registrationDate && selectedDate < registrationDate) {
        Alert.alert(t('error'), t('visitDateBeforeRegistration'));
        return;
      }
  ...
  const validateBloodPressure = bp => {
    ...
    if (!match) return t('bloodPressureFormatError');
    const systolic = parseInt(match[1]);
    const diastolic = parseInt(match[2]);
    if (systolic < 70 || systolic > 200) return t('systolicRangeError');
    if (diastolic < 40 || diastolic > 130) return t('diastolicRangeError');
    if (systolic <= diastolic) return t('systolicGreaterError');
    return null;
  };
  const validateFetalHeartRate = fhr => {
    if (!fhr) return null;
    const rate = parseFloat(fhr);
    if (isNaN(rate)) return t('fetalHeartRateNumberError');
    return null;
  };
  const validateHemoglobin = hb => {
    const level = parseFloat(hb);
    if (isNaN(level)) return t('hemoglobinNumberError');
    if (level < 4 || level > 20) return t('hemoglobinRangeError');
    return null;
  };
  const validateWeight = wt => {
    const weight = parseFloat(wt);
    if (isNaN(weight)) return t('weightNumberError');
    if (weight < 30 || weight > 200) return t('weightRangeError');
    return null;
  };
  const handleSave = async () => {
    ...
    try {
      ...
      const statusMessage = isOnline ? t('successfully') : t('syncWhenOnline');
              ...
              <View style={styles.formCard}>
                ...
                <View style={styles.formFields}>
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    color: '#6b7280'
                  }]} placeholder={t('autoFilled')} placeholderTextColor="#9ca3af" keyboardType="numeric" value={formData.visit_number} editable={false} />
                  </View>
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <TouchableOpacity style={styles.dateInput} onPress={() => setShowDatePicker(true)} activeOpacity={0.7}>
                      <Text style={formData.visit_date ? styles.dateText : styles.placeholderText}>
                        {formData.visit_date ? formatDateForDisplay(formData.visit_date) : t('selectDate')}
                      </Text>
                    </TouchableOpacity>
                  </View>
                  ...
                  <View style={styles.rowContainer}>
                    <View style={[styles.inputGroup, styles.halfWidth]}>
                      ...
                      <TextInput style={styles.input} placeholder={t('weightUnit')} placeholderTextColor="#9ca3af" keyboardType="decimal-pad" maxLength={5} value={formData.weight} onChangeText={value => {
                    ...
                    <View style={[styles.inputGroup, styles.halfWidth]}>
                      ...
                      <TextInput style={styles.input} placeholder="120/80" placeholderTextColor="#9ca3af" keyboardType="numbers-and-punctuation" value={formData.blood_pressure} onChangeText={value => {
                  ...
                  <View style={styles.rowContainer}>
                    <View style={[styles.inputGroup, styles.halfWidth]}>
                      <Text style={styles.inputLabel}>{t('hemoglobin')} (g/dL) *</Text>
                      <TextInput style={styles.input} placeholder="11.5" placeholderTextColor="#9ca3af" keyboardType="decimal-pad" value={formData.hemoglobin} onChangeText={value => {
                      let filtered = value.replace(/[^0-9.]/g, '');
                      const parts = filtered.split('.');
                    ...
                    <View style={[styles.inputGroup, styles.halfWidth]}>
                      ...
                      <TextInput style={styles.input} placeholder={t('fundalHeightUnit')} placeholderTextColor="#9ca3af" keyboardType="decimal-pad" value={formData.fundal_height} onChangeText={value => handleInputChange('fundal_height', value)} />
                    </View>
                  </View>
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <TextInput style={styles.input} placeholder="120-160" placeholderTextColor="#9ca3af" keyboardType="numeric" maxLength={3} value={formData.fetal_heart_rate} onChangeText={value => {
                    const filtered = value.replace(/[^0-9]/g, '');
                    setFormData({
                      ...formData,
                      fetal_heart_rate: filtered
                    });
                    if (filtered) {
                      const error = validateFetalHeartRate(filtered);
                      setErrors(prev => ({
                        ...prev,
                        fetal_heart_rate: error || ''
                      }));
                    } else {
                      setErrors(prev => ({
                        ...prev,
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <TextInput style={styles.input} placeholder={t('enterFacilityName')} placeholderTextColor="#9ca3af" value={formData.facility_name} onChangeText={value => handleInputChange('facility_name', value)} />
                  </View>

                  {/* Doctor Notes */}
                  <View style={styles.inputGroup}>
                    <Text style={styles.inputLabel}>{t('doctorNotes')}</Text>
                    <TextInput style={[styles.input, styles.textArea]} placeholder={t('enterNotes')} placeholderTextColor="#9ca3af" multiline numberOfLines={3} value={formData.doctor_notes} onChangeText={value => handleInputChange('doctor_notes', value)} />
                  </View>

                  {/* Next Visit Date */}
                  <View style={styles.inputGroup}>
                    <Text style={styles.inputLabel}>{t('nextVisitDate')}</Text>
                    <TouchableOpacity style={styles.dateInput} onPress={() => setShowNextDatePicker(true)} activeOpacity={0.7}>
                      <Text style={formData.next_visit_date ? styles.dateText : styles.placeholderText}>
                        {formData.next_visit_date ? formatDateForDisplay(formData.next_visit_date) : t('selectDate')}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </View>
                ...
                {showNextDatePicker && <DateTimePicker value={new Date()} mode="date" display="default" onChange={handleNextDateChange} minimumDate={new Date()} />}
                ...
                <View style={styles.buttonRow}>
                  ...
                  <TouchableOpacity style={[styles.button, styles.saveButton]} onPress={handleSave} activeOpacity={0.7}>
