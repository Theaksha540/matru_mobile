...
const DistrictDashboard = ({ navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      if (userData) {
        const user = userData;
        logger.log('User role loaded');
        setUserInfo(user);
        
        if (isOnline) {
          // Load fresh data when online
          const [overviewData, blocksData, summaryData, outcomeData, pointPerformanceData] = await Promise.all([
            dashboardAPI.getDistrictOverview(),
            user.district_id ? adminAPI.getBlocks(user.district_id) : Promise.resolve([]),
            reportsAPI.getDeliverySummary().catch(() => null),
            reportsAPI.getDeliveryOutcomeBreakdown().catch(() => null),
            reportsAPI.getDeliveryPointPerformance().catch(() => [])
          ]);
          ...
          setDpPerformance(
            Array.isArray(pointPerformanceData?.dp_performance)
              ? pointPerformanceData.dp_performance
              : Array.isArray(pointPerformanceData)
                ? pointPerformanceData
                : (pointPerformanceData?.items || [])
          );
          
          // Debug: Log the stats to see available fields
          console.log('District stats:', overviewData.overall_stats);
          
          await Promise.all([
            syncService.setCachedResource('district_overview', overviewData),
            syncService.setCachedResource('district_blocks', blocksData),
            syncService.setCachedResource('district_delivery_summary', summaryData),
            syncService.setCachedResource('district_delivery_outcome_breakdown', outcomeData),
            syncService.setCachedResource('district_dp_performance', pointPerformanceData),
          ]);
        } else {
          const [
            cachedOverview,
            ...
            syncService.getCachedResource('district_overview'),
            syncService.getCachedResource('district_blocks'),
            syncService.getCachedResource('district_delivery_summary'),
            syncService.getCachedResource('district_delivery_outcome_breakdown'),
            syncService.getCachedResource('district_dp_performance'),
          ]);
          ...
          if (cachedDPPerformance) {
            setDpPerformance(
              Array.isArray(cachedDPPerformance?.dp_performance)
                ? cachedDPPerformance.dp_performance
                : Array.isArray(cachedDPPerformance)
                  ? cachedDPPerformance
                  : (cachedDPPerformance?.items || [])
            );
          }
        }
      }
      ...
      try {
        ...
        logger.error('Error loading cached data:', cacheError);
      }
    } finally {
      setLoading(false);
    }
  ...
  const quickActions = [
    ...
    },
    {
      ...
      iconBgColor: '#8b45131a',
      iconColor: '#8B4513',
      onPress: () => navigation.navigate('DPReports', { scope: 'district' }),
    },
  ];
  ...
  const blockStats = blocks.map((block, index) => ({
    ...
    ancRate: block.completed_usg_appointments && block.total_usg_appointments 
      ? Math.round((block.completed_usg_appointments / block.total_usg_appointments) * 100) + '%'
      : '0%'
  }));
  const totalBlockPages = Math.max(1, Math.ceil(blockStats.length / blocksPerPage));
  const paginatedBlockStats = blockStats.slice(
    (currentBlockPage - 1) * blocksPerPage,
    currentBlockPage * blocksPerPage
  );
            ...
            <View style={[styles.contentContainer, { maxWidth: contentMaxWidth }]}>
              ...
              <View style={styles.dpKpiGrid}>
                {dpKpis.map((kpi) => (
                  ...
                  >
                    <Text style={[styles.dpKpiValue, { color: kpi.textColor }]}>{kpi.value}</Text>
                    <Text style={[styles.dpKpiLabel, { color: kpi.subTextColor }]}>{kpi.label}</Text>
                  </View>
                ))}
              </View>
              ...
              <View style={styles.blocksContainer}>
                {loading ? (
                  <Text style={styles.loadingText}>{t('loadingBlocks')}</Text>
                ) : blockStats.length > 0 ? (
                  <>
                    <ScrollView
                      style={styles.blocksScrollContainer}
                      showsVerticalScrollIndicator={true}
                      nestedScrollEnabled={true}
                    >
                      {paginatedBlockStats.map((block) => (
                        <TouchableOpacity
                          key={block.id}
                      ...
                      <View style={styles.paginationContainer}>
                        <TouchableOpacity
                          style={[
                            ...
                            currentBlockPage === 1 && styles.paginationButtonDisabled,
                          ]}
                          onPress={() => setCurrentBlockPage((prev) => Math.max(1, prev - 1))}
                        ...
                        <TouchableOpacity
                          style={[
                            styles.paginationButton,
                            currentBlockPage === totalBlockPages && styles.paginationButtonDisabled,
                          ]}
                          onPress={() => setCurrentBlockPage((prev) => Math.min(totalBlockPages, prev + 1))}
              ...
              <Text style={styles.sectionTitle}>
                DP-wise Overview / Delivery Point Summary
              </Text>
              <View style={styles.dpContainer}>
                ...
                {normalizedDPPerformance.length > 0 ? (
                  ...
                  <Text style={styles.noDataText}>No delivery point performance data found</Text>
                )}
              </View>

              {/* Real-time Monitoring Banner */}
              <LinearGradient
                colors={['#a9bfd6', '#fff4e6']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
