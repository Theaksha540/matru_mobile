...
const SubCentreDashboard = ({ navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      if (userData) {
        ...
        if (isOnline) {
          const highRiskPromise = pregnantWomenAPI.getAll({ is_high_risk: true });
          const totalRegisteredPromise = user.sub_centre_id
            ? pregnantWomenAPI.getAll({ sub_centre_id: user.sub_centre_id })
            : pregnantWomenAPI.getAll();

          const [statsData, pendingData, highRiskData, totalRegisteredData] = await Promise.all([
            dashboardAPI.getStats(),
            pregnantWomenAPI.getPendingApproval(),
            highRiskPromise,
          ...
          console.log('[SubCentreDashboard] High-risk KPI/QuickAction count:', highRiskCountValue);
          console.log('[SubCentreDashboard] Pending approval count:', filteredPending.length || 0);
          console.log('[SubCentreDashboard] Total registered count:', totalRegisteredValue);
          ...
          if (cachedTotalRegistered) {
            ...
            console.log('[SubCentreDashboard] Total registered count (cache):', cachedTotalValue);
          }
          if (cachedPending) {
            const pendingData = cachedPending;
            const pendingList = normalizeList(pendingData);
            setPendingCount(pendingList.length || 0);
            console.log('[SubCentreDashboard] Pending approval count (cache):', pendingList.length || 0);
          }
          if (cachedHighRisk) {
            const cachedHighRiskValue = Number(cachedHighRisk) || 0;
            setHighRiskCount(cachedHighRiskValue);
            console.log('[SubCentreDashboard] High-risk KPI/QuickAction count (cache):', cachedHighRiskValue);
          }
        }
      }
    } catch (error) {
  ...
  const quickActions = [
    ...
    },
    {
      ...
      iconColor: '#dc2626',
      onPress: () => navigation.navigate('PatientList', { action: 'createReferral' }),
    },
    {
      id: 7,
                ...
                {kpis.map((kpi) => {
                  if (kpi.id === 2 || kpi.id === 3) {
                    return (
                      <LinearGradient
                        key={kpi.id}
                        colors={kpi.id === 2 ? ['#fef2f2', '#fee2e2'] : ['#fff7ed', '#fed7aa']}
                        start={{ x: 0, y: 0 }}
                        end={{ x: 1, y: 1 }}
                        style={[
                          styles.kpiCard,
                      ...
                      >
                        ...
                        <Text style={[styles.kpiSubtitle, { color: kpi.textColor }]}>
                          {kpi.subtitle}
                        </Text>
                      </LinearGradient>
                    );
