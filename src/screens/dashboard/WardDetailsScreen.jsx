...
const WardDetailsScreen = ({ navigation, route }) => {
  const { t } = useTranslation();
  const isOnline = useNetworkStatus();
  const { wardId, wardName, blockId, blockName } = route.params;
  ...
  const syncDataForOffline = async (womenData, appointmentsData, centresData) => {
    try {
      ...
      console.error('Error syncing ward data for offline:', error);
    }
  };
    ...
    try {
      ...
      if (isOnline) {
        const [womenResponse, appointmentsResponse, centresResponse] = await Promise.allSettled([
          pregnantWomenAPI.getAll({ block_id: blockId, limit: 1000 }),
          usgAppointmentAPI.getAll({ limit: 1000 }),
          adminAPI.getUSGCentres().catch(() => []),
        ]);

        const womenData =
          womenResponse.status === 'fulfilled' ? ensureArray(womenResponse.value) : [];
        const appointmentsData =
          appointmentsResponse.status === 'fulfilled' ? ensureArray(appointmentsResponse.value) : [];
        const centresData =
          centresResponse.status === 'fulfilled' ? ensureArray(centresResponse.value) : [];

        const wardWomen = womenData.filter(
          (woman) => woman && String(woman.ward_id) === String(wardId)
        );
        const wardWomenIds = new Set(wardWomen.map((woman) => woman.id).filter(Boolean));
        const wardAppointments = appointmentsData.filter(
          (appointment) =>
            appointment &&
            appointment.pregnant_woman_id &&
            wardWomenIds.has(appointment.pregnant_woman_id)
        );
  ...
  const stats = [
    ...
    },
    {
      id: 4,
      value:
        wardData?.completed_usg_appointments && wardData?.total_usg_appointments
          ? `${Math.round((wardData.completed_usg_appointments / wardData.total_usg_appointments) * 100)}%`
          : '0%',
            ...
            <View style={styles.contentContainer}>
                ...
                {stats.map((stat) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text style={[styles.statTitle, { color: stat.textColor }]}>{stat.title}</Text>
                    </View>
                  );
                })}
              </View>
              ...
              {pregnantWomen.length > 0 && (
                <>
                  ...
                  >
                    <View style={styles.listContainer}>
                      {pregnantWomen
                        ...
                        .map((woman, index) => (
                          <View key={woman.id || index} style={styles.womanCard}>
                  ...
                  {Math.ceil(pregnantWomen.length / itemsPerPage) > 1 && (
                    <View style={styles.pagination}>
                      ...
                      <Text style={styles.pageInfo}>{t('pageOf', { current: womenPage, total: Math.ceil(pregnantWomen.length / itemsPerPage) })}</Text>
                      <TouchableOpacity
                        style={[styles.pageButton, womenPage === Math.ceil(pregnantWomen.length / itemsPerPage) && styles.disabledButton]}
                        onPress={() => setWomenPage(Math.min(Math.ceil(pregnantWomen.length / itemsPerPage), womenPage + 1))}
                        disabled={womenPage === Math.ceil(pregnantWomen.length / itemsPerPage)}
                      >
              ...
              {usgAppointments.length > 0 && (
                <>
                  ...
                  {Math.ceil(usgAppointments.length / itemsPerPage) > 1 && (
                    <View style={styles.pagination}>
                      <TouchableOpacity
                      ...
                      <Text style={styles.pageInfo}>{t('pageOf', { current: appointmentsPage, total: Math.ceil(usgAppointments.length / itemsPerPage) })}</Text>
                      <TouchableOpacity
                        style={[styles.pageButton, appointmentsPage === Math.ceil(usgAppointments.length / itemsPerPage) && styles.disabledButton]}
                        onPress={() => setAppointmentsPage(Math.min(Math.ceil(usgAppointments.length / itemsPerPage), appointmentsPage + 1))}
                        disabled={appointmentsPage === Math.ceil(usgAppointments.length / itemsPerPage)}
                      >
                        <Text style={styles.pageButtonText}>{t('next')}</Text>
                      </TouchableOpacity>
                    </View>
