...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, RefreshControl, ActivityIndicator, Alert, TextInput, Modal, Image, Platform, useWindowDimensions } from 'react-native';
import { SafeAreaView, SafeAreaProvider } from 'react-native-safe-area-context';
import { Menu, CheckCircle, Clock, Calendar, AlertTriangle, FileText, Bell } from 'lucide-react-native';
...
const USGDashboard = ({
  ...
  useEffect(() => {
    ...
    if (incomingSearch) {
      ...
      navigation.setParams({
        bookingSearchQuery: undefined,
        bookingAppointmentId: undefined
      });
      return;
    }
    if (Number.isFinite(incomingAppointmentId) && appointments.filtered.length > 0) {
      const matchedAppointment = appointments.filtered.find(apt => Number(apt?.id) === incomingAppointmentId);
      if (matchedAppointment) {
        const autoQuery = matchedAppointment?.patient?.mobile_number || matchedAppointment?.patient?.full_name || matchedAppointment?.patient?.address || String(matchedAppointment.id);
  ...
  const loadAppointments = async () => {
    try {
      ...
      const offlineCompletedIds = offlineReports.filter(r => r.sync_status === 'pending').map(r => r.appointment_id);
      ...
      const filteredAppointments = updatedData.filter(apt => apt.status === 'scheduled' || apt.status === 'accepted' || apt.status === 'rescheduled');
      ...
      const appointmentsWithPatients = await Promise.all(filteredAppointments.map(async apt => {
      ...
      setAppointments({
        all: allAppointments,
        filtered: appointmentsWithPatients
      });
    } catch (error) {
      setAppointments({
        all: [],
  ...
  const handleAccept = async appointmentId => {
    ...
    try {
      ...
      const errorMsg = error?.response?.data?.detail || error?.response?.data?.message || error?.message || 'Failed to accept appointment';
      Alert.alert('Error', errorMsg);
    }
  };
  const handleReschedule = appointmentId => {
    const selectedAppointmentData = appointments.filtered.find(apt => apt.id === appointmentId);
    const scheduledDate = selectedAppointmentData?.scheduled_date ? new Date(selectedAppointmentData.scheduled_date) : null;
  ...
  const submitReschedule = async () => {
    ...
    const originalScheduledDate = selectedAppointmentData?.scheduled_date ? new Date(selectedAppointmentData.scheduled_date) : null;
    if (originalScheduledDate && !Number.isNaN(originalScheduledDate.getTime())) {
      ...
      if (selectedDateTime > maxAllowedRescheduleDate) {
        Alert.alert('Error', `Reschedule date must be within 7 days of the current scheduled date (${formatDateDDMMYYYY(originalScheduledDate)}).`);
        return;
      }
    }
    try {
  ...
  const getTrimester = patient => {
    const today = new Date();
    const msPerDay = 1000 * 60 * 60 * 24;
    const lmpDate = patient?.lmp_date ? new Date(patient.lmp_date) : null;
    const eddDate = patient?.edd_date ? new Date(patient.edd_date) : patient?.expected_delivery_date ? new Date(patient.expected_delivery_date) : null;

    if (lmpDate && !Number.isNaN(lmpDate.getTime())) {
      const gestationalWeeks = Math.floor((today - lmpDate) / msPerDay / 7);
      if (gestationalWeeks <= 13) return '1st Trimester';
      if (gestationalWeeks <= 26) return '2nd Trimester';
      return '3rd Trimester';
    }

    if (eddDate && !Number.isNaN(eddDate.getTime())) {
      const weeksUntilEdd = Math.floor((eddDate - today) / msPerDay / 7);
      const gestationalWeeks = 40 - weeksUntilEdd;
      if (gestationalWeeks <= 13) return '1st Trimester';
      if (gestationalWeeks <= 26) return '2nd Trimester';
      return '3rd Trimester';
    }
  ...
  const getRequestedByDisplayName = appointment => {
    const candidates = [appointment?.requested_by_name, appointment?.requested_by_user_name, appointment?.requested_by_full_name, appointment?.created_by_name, appointment?.scheduled_by_name, appointment?.sub_centre_user_name, appointment?.anm_name, appointment?.requested_by];
  ...
  const completedToday = appointments.all.filter(apt => apt.status === 'completed' && apt.completed_date && new Date(apt.completed_date).toDateString() === today).length;
  ...
  const filteredBookingRequests = appointments.filtered.filter(apt => {
    ...
    return apt.patient?.full_name?.toLowerCase().includes(query) || apt.patient?.mobile_number?.includes(query) || apt.patient?.address?.toLowerCase().includes(query);
  });
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
        ...
        paddingHorizontal: horizontalPadding
          ...
          <View style={styles.statsGrid}>
            ...
            <LinearGradient colors={['#fff7ed', '#fed6aa']} start={{
            x: 0,
            y: 0
          }} end={{
            x: 1,
          ...
          {overdueAppointments > 0 && <View style={styles.overdueCard}>
          ...
          <View style={styles.section}>
            ...
            {/* Search Bar */}
            <TextInput style={styles.searchInput} placeholder="Search by name, mobile, village..." placeholderTextColor="#9ca3af" value={searchQuery} onChangeText={setSearchQuery} />
            
            {filteredBookingRequests.length === 0 ? <View style={styles.emptyState}>
                <Text style={styles.emptyText}>No pending requests</Text>
              </View> : <View>
              {paginatedBookingRequests.map(apt => {
              const priority = getPriorityInfo(apt.patient, apt);
              return <View key={apt.id} style={styles.requestCard}>
                  ...
                  appointmentId: apt.id
                    ...
                    <View style={styles.requestHeader}>
                      <View style={styles.priorityBadge}>
                        ...
                        {apt.patient?.is_high_risk && <AlertTriangle size={14} color="#f59e0b" />}
                      </View>
                    </View>
                    <Text style={styles.requestName}>{apt.patient?.full_name || 'N/A'}</Text>
                    <Text style={styles.requestDetails}>
                      Age: {apt.patient?.age || 'N/A'} years • {getTrimester(apt.patient)}{"\n"}
                    ...
                    {apt.reschedule_count > 0 && <Text style={styles.rescheduleInfo}>
                        ⚠ Rescheduled {apt.reschedule_count} time(s)
                      </Text>}
                    {isAppointmentOverdue(apt) && <View style={styles.overdueBadge}>
                        <AlertTriangle size={12} color="#ef4444" />
                        <Text style={styles.overdueLabel}>{t('overdue')}</Text>
                      </View>}
                      <Text style={styles.requestedBy}>Requested by: {getRequestedByDisplayName(apt)}</Text>
                      {apt.appointment_type === 'emergency' && <View style={styles.emergencyBanner}>
                          <AlertTriangle size={12} color="#ef4444" />
                        <Text style={styles.emergencyText}>{t('emergencyCase')}</Text>
                      </View>}
                    <View style={styles.actionButtons}>
                      {apt.status === 'accepted' ? <View style={styles.acceptedBadge}>
                          <CheckCircle size={14} color="#22c55e" style={{
                        marginRight: 4
                      }} />
                          <Text style={styles.acceptedText}>Accepted</Text>
                        </View> : apt.status === 'rescheduled' ? <View style={styles.rescheduledBadge}>
                          <Clock size={14} color="#f59e0b" style={{
                        marginRight: 4
                      }} />
                          <Text style={styles.rescheduledText}>Rescheduled</Text>
                        </View> : <>
                          <TouchableOpacity style={styles.acceptButton} onPress={() => handleAccept(apt.id)}>
                            <CheckCircle size={14} color="white" style={{
                          marginRight: 4
                        }} />
                            <Text style={styles.acceptButtonText}>Accept</Text>
                          </TouchableOpacity>
                          {apt.appointment_type !== 'emergency' && <TouchableOpacity style={styles.rescheduleButton} onPress={() => handleReschedule(apt.id)}>
                              <Clock size={14} color="white" style={{
                          marginRight: 4
                        }} />
            ...
            {filteredBookingRequests.length > 0 && <View style={styles.paginationContainer}>
                <TouchableOpacity style={[styles.pageButton, currentPage === 1 && styles.pageButtonDisabled]} onPress={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1}>
                  <Text style={[styles.pageButtonText, currentPage === 1 && styles.pageButtonTextDisabled]}>Previous</Text>
                </TouchableOpacity>
                <Text style={styles.pageInfo}>Page {currentPage} of {totalPages}</Text>
                <TouchableOpacity style={[styles.pageButton, currentPage === totalPages && styles.pageButtonDisabled]} onPress={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages}>
          ...
          <View style={styles.section}>
            ...
            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('CompletedReports')}>
            ...
            <TouchableOpacity style={styles.actionItem} onPress={() => navigation.navigate('AllReports')}>
              <View style={styles.actionIcon}>
        ...
        <Modal visible={showRescheduleModal} transparent animationType="slide">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <TextInput style={styles.modalInput} placeholder="Reason for reschedule" value={rescheduleReason} onChangeText={setRescheduleReason} multiline />
              <View style={styles.modalButtons}>
                ...
                <TouchableOpacity style={styles.modalSubmitButton} onPress={submitReschedule}>
