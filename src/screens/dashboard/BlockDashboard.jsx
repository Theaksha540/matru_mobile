...
import { Menu, UserPlus, TriangleAlert, ClipboardList, FileText, Bell, Download, MapPin } from 'lucide-react-native';
...
const BlockDashboard = ({
    ...
    try {
      ...
      if (userData) {
        ...
        if (isOnline) {
          ...
          const reportParams = effectiveBlockId ? {
            block_id: effectiveBlockId
          } : {};
          // Load stats for specific block if blockId provided
          const [statsData, summaryData, outcomeData, pointPerformanceData, wardTrendData, grievanceData] = await Promise.all([effectiveBlockId ? dashboardAPI.getBlockStats(effectiveBlockId).catch(statsError => {
            return null;
            ...
            return [];
          }), effectiveBlockId ? reportsAPI.getWardWiseTrends(effectiveBlockId).catch(wardError => {
            return [];
          }) : Promise.resolve([]), grievanceAPI.getAll(reportParams).catch(grievanceError => {
            return [];
          })]);
          ...
          setDpPerformance(extractListFromResponse(pointPerformanceData, ['dp_performance', 'delivery_points']));
          setWardPerformance(extractListFromResponse(wardTrendData, ['wards', 'ward_wise_trends', 'ward_trends', 'trends', 'results']));
          const cacheKey = effectiveBlockId ? `cached_block_stats_${effectiveBlockId}` : 'cached_block_stats';
          await Promise.all([resolvedStats ? syncService.setCachedResource(cacheKey, resolvedStats) : Promise.resolve(false), summaryData ? syncService.setCachedResource(`${cacheKey}_delivery_summary`, summaryData) : Promise.resolve(false), outcomeData ? syncService.setCachedResource(`${cacheKey}_delivery_outcome_breakdown`, outcomeData) : Promise.resolve(false), pointPerformanceData ? syncService.setCachedResource(`${cacheKey}_dp_performance`, pointPerformanceData) : Promise.resolve(false), wardTrendData ? syncService.setCachedResource(`${cacheKey}_ward_performance`, wardTrendData) : Promise.resolve(false), grievanceData ? syncService.setCachedResource(`${cacheKey}_grievances`, grievanceData) : Promise.resolve(false)]);
        } else {
          const effectiveBlockId = blockId || user?.block_id;
          const cacheKey = effectiveBlockId ? `cached_block_stats_${effectiveBlockId}` : 'cached_block_stats';
          const [cachedStats, cachedDeliverySummary, cachedDeliveryOutcomeBreakdown, cachedDPPerformance, cachedWardPerformance, cachedGrievances] = await Promise.all([syncService.getCachedResource(cacheKey), syncService.getCachedResource(`${cacheKey}_delivery_summary`), syncService.getCachedResource(`${cacheKey}_delivery_outcome_breakdown`), syncService.getCachedResource(`${cacheKey}_dp_performance`), syncService.getCachedResource(`${cacheKey}_ward_performance`), syncService.getCachedResource(`${cacheKey}_grievances`)]);
          if (cachedStats) {
          ...
          if (cachedDPPerformance) {
            setDpPerformance(extractListFromResponse(cachedDPPerformance, ['dp_performance', 'delivery_points']));
          }
          if (cachedWardPerformance) {
            setWardPerformance(extractListFromResponse(cachedWardPerformance, ['wards', 'ward_wise_trends', 'ward_trends', 'trends', 'results']));
          }
          if (cachedGrievances) {
  ...
  const normalizeStatsResponse = response => {
    ...
    if (response.total_pregnant_women !== undefined || response.high_risk_cases !== undefined || response.pending_usg_appointments !== undefined || response.completed_usg_appointments !== undefined) {
      return response;
    }
  ...
  const buildFallbackBlockStats = async effectiveBlockId => {
    ...
    try {
      const [womenResponse, appointmentsResponse] = await Promise.all([pregnantWomenAPI.getAll({
        block_id: effectiveBlockId,
        limit: 1000
      ...
      const blockWomen = womenData.filter(woman => woman && String(woman.block_id) === String(effectiveBlockId));
      const blockWomenIds = new Set(blockWomen.map(woman => woman?.id).filter(Boolean));
      const blockAppointments = appointmentsData.filter(appointment => appointment && appointment.pregnant_woman_id && blockWomenIds.has(appointment.pregnant_woman_id));
      return {
  ...
  const kpis = [{
    ...
    id: 4,
    value: stats?.completed_usg_appointments && stats?.total_usg_appointments ? `${Math.round(stats.completed_usg_appointments / stats.total_usg_appointments * 100)}%` : '0%',
  ...
  const normalizedWardPerformance = wardPerformance.map((item, index) => {
    ...
    const totalRegistrations = monthlyData.length ? monthlyData.reduce((sum, month) => sum + toSafeNumber(month?.registrations), 0) : toSafeNumber(item?.registrations ?? item?.total_registrations ?? item?.total_cases ?? item?.registered_cases);
    const totalHighRisk = monthlyData.length ? monthlyData.reduce((sum, month) => sum + toSafeNumber(month?.high_risk), 0) : toSafeNumber(item?.high_risk ?? item?.high_risk_cases);
    const totalAppointments = monthlyData.length ? monthlyData.reduce((sum, month) => sum + toSafeNumber(month?.appointments), 0) : toSafeNumber(item?.appointments ?? item?.anc_visits ?? item?.total_appointments);
    const totalDeliveries = monthlyData.length ? monthlyData.reduce((sum, month) => sum + toSafeNumber(month?.deliveries), 0) : toSafeNumber(item?.deliveries ?? item?.completed_deliveries ?? item?.total_deliveries);
    const currentMonth = monthlyData[0] || {};
    const previousMonth = monthlyData[1] || {};
    const trendValue = previousMonth?.registrations ? (toSafeNumber(currentMonth?.registrations) - toSafeNumber(previousMonth?.registrations)) / toSafeNumber(previousMonth?.registrations) * 100 : toSafeNumber(item?.trend ?? item?.trend_percentage);
    return {
  ...
  const paginatedWardPerformance = normalizedWardPerformance.slice((currentWardPage - 1) * wardsPerPage, currentWardPage * wardsPerPage);
  useEffect(() => {
          ...
          paddingHorizontal: horizontalPadding
            ...
            maxWidth: contentMaxWidth
              ...
              <View style={styles.dpKpiGrid}>
                {dpKpis.map(kpi => <View key={kpi.id} style={[styles.kpiCard, {
                width: '48%',
                backgroundColor: kpi.bgColor,
                borderColor: kpi.borderColor
              }]}>
              ...
              <View style={styles.dpContainer}>
                {normalizedWardPerformance.length > 0 ? <>
                    <View>
                      {paginatedWardPerformance.map(ward => <TouchableOpacity key={ward.id} style={styles.blockCard} onPress={() => handleWardSelect(ward)} activeOpacity={0.7}>
                    ...
                    {totalWardPages > 1 && <View style={styles.paginationContainer}>
                        <TouchableOpacity style={[styles.paginationButton, currentWardPage === 1 && styles.paginationButtonDisabled]} onPress={() => setCurrentWardPage(prev => Math.max(1, prev - 1))} disabled={currentWardPage === 1}>
                        ...
                        <View style={styles.paginationInfo}>
                          <Text style={styles.paginationText}>
                            {t('pageOf', {
                        current: currentWardPage,
                        total: totalWardPages
                      })}
                          </Text>
                        ...
                        <TouchableOpacity style={[styles.paginationButton, currentWardPage === totalWardPages && styles.paginationButtonDisabled]} onPress={() => setCurrentWardPage(prev => Math.min(totalWardPages, prev + 1))} disabled={currentWardPage === totalWardPages}>
