...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Image, ActivityIndicator, Alert, RefreshControl, TextInput } from 'react-native';
...
const BlockDetailsScreen = ({
  ...
  const filteredPregnantWomen = pregnantWomen.filter(woman => {
    ...
    return [woman?.full_name, woman?.name, woman?.rch_id, woman?.registration_no, woman?.mobile_number, woman?.phone].filter(Boolean).some(value => String(value).toLowerCase().includes(query));
  });
  const filteredUsgAppointments = usgAppointments.filter(appointment => {
    const query = appointmentsSearchQuery.trim().toLowerCase();
    if (!query) return true;
    return [patients[appointment?.pregnant_woman_id], appointment?.status, appointment?.appointment_type, appointment?.centre_name, usgCentres[appointment?.usg_centre_id], appointment?.notes].filter(Boolean).some(value => String(value).toLowerCase().includes(query));
  });
  ...
  const loadBlockData = async (showRefresh = false) => {
    try {
      ...
      if (isOnline) {
        ...
        const blockWomen = womenData.filter(w => w && w.block_id?.toString() === blockId?.toString());

        // Filter appointments for these women
        const blockWomenIds = new Set(blockWomen.map(w => w.id));
        const blockAppointments = appointmentsData.filter(a => a && a.pregnant_woman_id && blockWomenIds.has(a.pregnant_woman_id));
        ...
        // Filter by blockId
        const blockWomen = womenData.filter(w => w && w.block_id && w.block_id.toString() === blockId.toString());
        const blockWomenIds = new Set(blockWomen.map(w => w.id));
        const blockAppointments = appointmentsData.filter(a => a && a.pregnant_woman_id && blockWomenIds.has(a.pregnant_woman_id));
        // Update state
        setPregnantWomen(blockWomen);
      ...
      Alert.alert('Error', 'Failed to load block data. Please try again.');
  ...
  const stats = [{
    ...
    id: 4,
    value: blockData?.completed_usg_appointments && blockData?.total_usg_appointments ? `${Math.round(blockData.completed_usg_appointments / blockData.total_usg_appointments * 100)}%` : '0%',
            ...
            <View style={styles.headerContent}>
              ...
              <View style={styles.headerInfo}>
                ...
                <View style={styles.statusContainer}>
                  ...
                  {syncStatus === 'syncing' && <View style={styles.syncBadge}>
                      <ActivityIndicator size="small" color="#D2691E" />
                      <Text style={styles.syncText}>Syncing...</Text>
                    </View>}
                  {syncStatus === 'synced' && <View style={[styles.syncBadge, styles.syncedBadge]}>
                      <Text style={styles.syncedText}>✓ Synced</Text>
                    </View>}
                  {syncStatus === 'no_data' && !isOnline && <View style={[styles.syncBadge, styles.noDataBadge]}>
            ...
            <View style={styles.contentContainer}>
              ...
              {pregnantWomen.length > 0 && <>
                  ...
                  <View style={styles.searchContainer}>
                    ...
                    <TextInput style={styles.searchInput} placeholder="Search pregnant women..." placeholderTextColor="#9ca3af" value={womenSearchQuery} onChangeText={setWomenSearchQuery} />
                  </View>
                  
                  <View style={styles.listContainer}>
                    <ScrollView style={styles.innerListScroll} showsVerticalScrollIndicator={true} nestedScrollEnabled={true}>
                      {filteredPregnantWomen.slice((womenPage - 1) * itemsPerPage, womenPage * itemsPerPage).map((woman, index) => <TouchableOpacity key={woman.id || index} style={styles.womanCard} onPress={() => handleWomanPress(woman)} activeOpacity={0.7}>
                  ...
                  {/* Pagination */}
                  {Math.ceil(filteredPregnantWomen.length / itemsPerPage) > 1 && <View style={styles.pagination}>
                      ...
                      <TouchableOpacity style={[styles.pageButton, womenPage === Math.ceil(filteredPregnantWomen.length / itemsPerPage) && styles.disabledButton]} onPress={() => setWomenPage(Math.min(Math.ceil(filteredPregnantWomen.length / itemsPerPage), womenPage + 1))} disabled={womenPage === Math.ceil(filteredPregnantWomen.length / itemsPerPage)}>
              ...
              {usgAppointments.length > 0 && <>
                  ...
                  <View style={styles.searchContainer}>
                    <Search size={18} color="#6b7280" style={styles.searchIcon} />
                    <TextInput style={styles.searchInput} placeholder="Search USG appointments..." placeholderTextColor="#9ca3af" value={appointmentsSearchQuery} onChangeText={setAppointmentsSearchQuery} />
                  </View>
                  
                  <View style={styles.listContainer}>
                    <ScrollView style={styles.innerListScroll} showsVerticalScrollIndicator={true} nestedScrollEnabled={true}>
                      {filteredUsgAppointments.slice((appointmentsPage - 1) * itemsPerPage, appointmentsPage * itemsPerPage).map((appointment, index) => <TouchableOpacity key={appointment.id || index} style={styles.appointmentCard} onPress={() => handleAppointmentPress(appointment)} activeOpacity={0.7}>
                        ...
                        backgroundColor: appointment.status === 'pending' ? '#fef3c7' : appointment.status === 'completed' ? '#dcfce7' : appointment.status === 'cancelled' ? '#fee2e2' : '#f3f4f6'
                            ...
                            <View style={styles.appointmentTypeRow}>
                              <Text style={styles.appointmentDetailLabel}>Type:</Text>
                              <View style={[styles.appointmentTypeBadge, appointment.appointment_type === 'emergency' ? styles.emergencyTypeBadge : styles.regularTypeBadge]}>
                                <Text style={styles.appointmentTypeBadgeText}>
                                  {(appointment.appointment_type || 'Regular').toString().replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, char => char.toUpperCase())}
                                </Text>
                              </View>
                            </View>
                            <Text style={styles.appointmentDetail}>
                              Centre: {usgCentres[appointment.usg_centre_id] || appointment.centre_name || `Centre ${appointment.usg_centre_id}`}
                            </Text>
                            {appointment.notes && <Text style={styles.appointmentNotes}>
                  ...
                  {/* Pagination */}
                  {Math.ceil(filteredUsgAppointments.length / itemsPerPage) > 1 && <View style={styles.pagination}>
                      ...
                      <TouchableOpacity style={[styles.pageButton, appointmentsPage === Math.ceil(filteredUsgAppointments.length / itemsPerPage) && styles.disabledButton]} onPress={() => setAppointmentsPage(Math.min(Math.ceil(filteredUsgAppointments.length / itemsPerPage), appointmentsPage + 1))} disabled={appointmentsPage === Math.ceil(filteredUsgAppointments.length / itemsPerPage)}>
                        <Text style={styles.pageButtonText}>Next</Text>
                      </TouchableOpacity>
                    </View>}
                </>}
