...
const DPDashboard = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const refreshInterval = setInterval(() => {
      if (isOnline) {
        console.log('Auto-refreshing DP Dashboard data...');
        loadData();
      }
    }, 5 * 60 * 1000); // 5 minutes
  ...
  const loadData = async () => {
    try {
      ...
      setUserInfo(user);
      console.log('DP Dashboard - Loading data for user:', user);
      ...
      if (isOnline) {
        try {
          ...
          try {
            ...
            console.log('District overview API not available for DP role:', overviewError);
          }
        } catch (apiError) {
          console.error('API failed, loading cached data:', apiError);
      ...
      logger.error('Error loading DP dashboard data:', error);
      await loadCachedData();
  ...
  const loadReferralData = async () => {
    try {
      console.log('Loading real-time referral data...');
      
      // Load counts for each status using the same approach as DeliveryReferralsScreen
      const [allData, pendingData, acceptedData, reReferredData, completedData, recentReferrals] = await Promise.all([
        deliveryReferralAPI.getAll(),
        deliveryReferralAPI.getAll({ status: 'pending' }),
        deliveryReferralAPI.getAll({ status: 'accepted' }),
        deliveryReferralAPI.getAll({ status: 're_referred' }),
        deliveryReferralAPI.getAll({ status: 'completed' }),
        deliveryReferralAPI.getAll()
      ]);
      ...
      console.log('DP Dashboard - Real-time stats loaded:', calculatedStats);
      console.log('DP Dashboard - Recent referrals loaded:', recentReferralsList.length);
      
      setReferralStats(calculatedStats);
      setDeliveryReferrals(recentReferralsList);
      
      await Promise.all([
        syncService.setCachedResource('dp_referral_stats', calculatedStats),
        syncService.setCachedResource('dp_referrals', recentReferralsList),
      ]);
  ...
  const loadCachedData = async () => {
    try {
      const [cachedOverview, cachedReferrals, cachedReferralStats] = await Promise.all([
        syncService.getCachedResource('dp_overview'),
        syncService.getCachedResource('dp_referrals'),
        syncService.getCachedResource('dp_referral_stats')
      ]);
      ...
      console.log('DP Dashboard - Loaded cached data');
    } catch (error) {
  ...
  const quickActions = [
    {
      id: 0,
      title: 'Create Delivery Referral',
      subtitle: 'Search and refer pregnant women for delivery',
      icon: UserPlus,
      onPress: () => {
        console.log('=== CREATE DELIVERY REFERRAL CLICKED ===');
        console.log('Navigating to PatientList with action: createReferral');
        navigation.navigate('PatientList', { action: 'createReferral' });
      },
      isPrimary: true,
    },
    {
                    ...
                    >
                      <View style={styles.actionContent}>
                        <View style={[
                          ...
                          action.isPrimary && styles.primaryActionIconContainer
                        ]}>
                        ...
                        <View style={styles.actionTextContainer}>
                          <Text style={[
                            styles.actionTitle,
                            action.isPrimary && styles.primaryActionTitle
                          ]}>{action.title}</Text>
                          <Text style={[
                            styles.actionSubtitle,
                            action.isPrimary && styles.primaryActionSubtitle
                  ...
                  deliveryReferrals.slice(0, 5).map((referral) => (
                    ...
                    >
                        ...
                        <View style={[
                          styles.statusBadge,
                          {
                            backgroundColor: 
                              ...
                              referral.status === 'accepted' ? '#d1fae5' :
                              referral.status === 're_referred' ? '#fee2e2' :
                              referral.status === 'completed' ? '#dbeafe' : '#f3f4f6'
                          }
                        ]}>
                          <Text style={[
                            styles.statusText,
                            {
                              color: 
                                referral.status === 'pending' ? '#d97706' :
                                referral.status === 'accepted' ? '#059669' :
                                referral.status === 're_referred' ? '#dc2626' :
                                referral.status === 'completed' ? '#2563eb' : '#6b7280'
                            }
                          ]}>
                      ...
                      {referral.referral_chain_summary && (
                        <Text style={styles.referralNotes} numberOfLines={1}>
                          Chain: {referral.referral_chain_summary}
                        </Text>
                      )}
                      {referral.observation_notes && (
                        <Text style={styles.referralNotes} numberOfLines={2}>
                          {referral.observation_notes}
                        </Text>
                      )}
                    </TouchableOpacity>
