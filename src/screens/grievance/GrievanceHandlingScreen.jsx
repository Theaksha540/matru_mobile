...
const GrievanceHandlingScreen = ({ navigation }) => {
  ...
  const loadLookupData = async (user) => {
    try {
      const [blocksData, wardsData] = await Promise.all([
        user?.district_id ? adminAPI.getBlocks(user.district_id).catch(() => []) : adminAPI.getBlocks().catch(() => []),
        user?.block_id ? adminAPI.getWards({ block_id: user.block_id }).catch(() => []) : adminAPI.getWards().catch(() => []),
      ]);
      ...
      console.error('Error loading grievance lookup data:', error);
    }
  };
  ...
  const loadData = async (currentUser = userInfo) => {
    try {
      ...
      setStatistics({ total_grievances: 0, pending: 0, resolved: 0, in_progress: 0, escalated: 0 });
      setGrievances([]);
    } finally {
      setLoading(false);
    }
  ...
  const tabs = statistics ? [
    { label: 'All', count: statistics.total_grievances || 0 },
    { label: 'Pending', count: statistics.pending || 0 },
    { label: 'In Progress', count: statistics.in_progress || 0 },
    { label: 'Resolved', count: statistics.resolved || 0 },
    { label: 'Escalated', count: statistics.escalated || 0 },
  ] : [
    { label: 'All', count: 0 },
  ...
  const handleResolve = async () => {
    if (isOnline) {
      ...
      const updatedGrievances = grievances.map(g => 
        g.id === selectedGrievanceId ? { ...g, status: 'resolved' } : g
      );
  ...
  const handleMarkInProgress = async (grievanceId) => {
    if (isOnline) {
      try {
        await grievanceAPI.updateStatus(grievanceId, { status: 'in_progress' });
        loadData();
      } catch (error) {
      ...
      // Update local grievance status
      const updatedGrievances = grievances.map(g => 
        g.id === grievanceId ? { ...g, status: 'in_progress' } : g
      );
      setGrievances(updatedGrievances);
  ...
  const handleRefresh = async () => {
    if (!isOnline) {
      Alert.alert(
        'Offline Mode',
        'Cannot refresh data while offline. Please connect to the internet.',
        [{ text: 'OK' }]
      );
      return;
    }
            ...
            <View style={styles.contentContainer}>
              ...
              <View style={styles.tabsContainer}>
                {tabs.map((tab) => (
                  <TouchableOpacity
                    key={tab.label}
                    style={[styles.tabButton, activeTab === tab.label && styles.tabButtonActive]}
              ...
              <View style={styles.grievancesList}>
                {loading ? (
                  ...
                  <View style={styles.emptyState}>
                    ...
                    <Text style={styles.emptyStateSubtext}>
                      {isOnline 
                        ...
                        : 'Please connect to the internet to fetch latest data'
                      }
                    </Text>
                    {!isOnline && (
                      <TouchableOpacity 
                  ...
                  filteredGrievances.map((grievance) => (
                    <TouchableOpacity 
                      key={grievance.id || grievance.ticket_number} 
                    ...
                    >
                      <View style={styles.grievanceCardHeader}>
                        ...
                        <View style={[
                          ...
                          grievance.escalated_to_district && styles.statusEscalated,
                        ]}>
                          ...
                          {grievance.escalated_to_district && userInfo?.role === 'block' ? (
                            ...
                            <>
                              <TouchableOpacity 
                                ...
                                activeOpacity={0.7}
                                onPress={() => handleMarkInProgress(grievance.id)}
