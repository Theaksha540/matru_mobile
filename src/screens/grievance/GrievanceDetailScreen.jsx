...
import { ArrowLeft, FileText, Calendar, Phone, MapPin, User, AlertCircle, CheckCircle, Home } from 'lucide-react-native';
...
const GrievanceDetailScreen = ({
  ...
  const loadGrievanceDetail = async () => {
    try {
      ...
      if (data) {
        ...
        const [districts, blocks] = await Promise.all([syncService.getDistricts(), syncService.getBlocks()]);
  ...
  const handleMarkInProgress = async () => {
    ...
    if (isOnline) {
      try {
        await grievanceAPI.updateStatus(grievance.id, {
          status: 'in_progress'
        });
          ...
          <View style={styles.mainContent}>
            <View style={styles.contentContainer}>
              ...
              <View style={styles.statusCard}>
                <View style={[styles.statusBadge, grievance.status === 'pending' && styles.statusPending, grievance.status === 'in_progress' && styles.statusInProgress, grievance.status === 'resolved' && styles.statusResolved]}>
                  <Text style={styles.statusBadgeText}>{grievance.status}</Text>
                </View>
                {grievance.escalated_to_district && <View style={styles.escalationTag}>
              ...
              {/* Notes Card */}
              {grievance.notes && <View style={styles.card}>
                  <View style={styles.cardHeader}>
                    <FileText size={18} color="#D2691E" />
              ...
              {/* Resolution Card */}
              {grievance.resolution_note && <View style={[styles.card, styles.resolutionCard]}>
                  <View style={styles.cardHeader}>
              ...
              <View style={styles.card}>
                ...
                {grievance.rch_id && <View style={styles.infoItem}>
              ...
              <View style={styles.card}>
                ...
                {grievance.ward_id && <View style={styles.infoRow}>
              ...
              <View style={styles.card}>
                ...
                {grievance.resolved_at && <View style={styles.timelineItem}>
                    <View style={[styles.timelineDot, {
              ...
              {grievance.status !== 'resolved' && <View style={styles.bottomActions}>
                  {grievance.escalated_to_district && userInfo?.role === 'block' ? <Text style={styles.escalatedNote}>Only district can handle escalated grievances</Text> : <View style={styles.grievanceActions}>
                      <TouchableOpacity style={[styles.btnSecondary, grievance.status === 'in_progress' && styles.btnDisabled]} activeOpacity={0.7} onPress={handleMarkInProgress} disabled={grievance.status === 'in_progress'}>
                      ...
                      <TouchableOpacity style={styles.btnPrimary} activeOpacity={0.7} onPress={() => setShowResolveModal(true)}>
