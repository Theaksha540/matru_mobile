...
const AllReportsScreen = ({
  ...
  const {
    ...
    refresh,
    updateParams
  } = useServerPagination(usgAppointmentAPI.getAll, getApiParams(), 10 // page size
  );
  useEffect(() => {
    loadUserRole();
  }, []);
  ...
  useEffect(() => {
    const loadPatientData = async () => {
      ...
      if (filter === 'overdue') {
        ...
        filtered = reportsWithPatientData.filter(report => new Date(report.created_at) >= weekAgo);
      } else if (filter === 'month') {
        const monthAgo = new Date();
        monthAgo.setMonth(monthAgo.getMonth() - 1);
        filtered = reportsWithPatientData.filter(report => new Date(report.created_at) >= monthAgo);
      }
            ...
            {reportsWithPatients.map(report => <View key={report.id} style={styles.reportCard}>
                ...
                <View style={styles.reportFooter}>
                  ...
                  <Text style={styles.completedDate}>
                    {userRole === 'usg_centre' ? `${t('scheduled')}: ${formatDate(report.scheduled_date)}` : `${t('created')}: ${formatDate(report.created_at)} • ${t('scheduled')}: ${formatDate(report.scheduled_date)}`}
                    {report.status === 'completed' && report.completed_date && ` • ${t('completed')}: ${formatDate(report.completed_date)}`}
                  </Text>
                </View>
              </View>)}
