...
const DeliveryPointReportsScreen = ({ navigation, route }) => {
  ...
  const loadData = async (params = {}) => {
    try {
      ...
      if (isOnline) {
        const [summary, outcomeBreakdown, performance] = await Promise.all([
          reportsAPI.getDeliverySummary(params),
          reportsAPI.getDeliveryOutcomeBreakdown(params),
          reportsAPI.getDeliveryPointPerformance(params),
        ]);
        ...
        await Promise.all([
          syncService.setCachedResource(`${cachePrefix}_summary`, summary),
          syncService.setCachedResource(`${cachePrefix}_outcomes`, outcomeBreakdown),
          syncService.setCachedResource(`${cachePrefix}_performance`, performance),
        ]);
      } else {
        const [cachedSummary, cachedOutcomes, cachedPerformance] = await Promise.all([
          syncService.getCachedResource(`${cachePrefix}_summary`),
          syncService.getCachedResource(`${cachePrefix}_outcomes`),
          syncService.getCachedResource(`${cachePrefix}_performance`),
        ]);
      ...
      console.error('Error loading DP reports:', error);
  ...
  const buildCsv = () => {
    ...
    const summaryRows = [
      ['Filter', getFilterFileSuffix(selectedFilter)],
      ['Start Date', activeParams.start_date || 'All'],
      ['End Date', activeParams.end_date || 'All'],
      [],
      ['Metric', 'Value'],
      ['Total Referrals', referralSummary.total_referrals ?? summarySource.totalReferrals],
      ['Pending Referrals', referralSummary.pending ?? 0],
      ['Accepted Referrals', referralSummary.accepted ?? summarySource.accepted],
      ['Re-Referred', referralSummary.re_referred ?? summarySource.reReferred],
      ['Completed Referrals', referralSummary.completed ?? summarySource.completed],
      ['Total Outcomes', outcomeSummary.total_outcomes ?? 0],
      ['Adverse Outcome Rate', outcomeSummary.adverse_outcome_rate ?? 0],
    ];

    const performanceRows = [
      [],
      ['DP-wise Overview'],
      ['DP Name', 'Referrals', 'Accepted', 'Re-Referred', 'Completed', 'Acceptance Rate'],
      ...pointPerformance.map((item) => [
        item?.dp_name || 'Unknown',
        item?.total_referrals_received || 0,
        item?.accepted || 0,
        item?.re_referred || 0,
        item?.completed || 0,
        formatRate(item?.acceptance_rate),
      ]),
    ];

    return [...summaryRows, ...performanceRows]
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      if (error.message.includes('Downloads folder access was not granted')) {
        Alert.alert(
          'Permission Required',
          'Download folder access is required to save reports. Please grant permission when prompted.',
          [{ text: 'OK' }]
        );
      } else {
  ...
  const derivedSummary = pointPerformance.reduce(
    (acc, item) => {
    ...
    { totalReferrals: 0, accepted: 0, reReferred: 0, completed: 0 }
  );
  ...
  const summaryStats = [
    {
      id: 1,
      value: String(
        summaryData?.referral_summary?.total_referrals ??
        summaryData?.total_referrals ??
        summaryData?.referrals ??
        derivedSummary.totalReferrals ??
        0
      ),
      label: 'Referrals',
    },
    {
      id: 2,
      value: String(
        summaryData?.referral_summary?.accepted ??
        summaryData?.accepted_referrals ??
        summaryData?.accepted ??
        derivedSummary.accepted ??
        0
      ),
      label: 'Accepted',
    },
    {
      id: 3,
      value: String(
        summaryData?.outcome_summary?.total_outcomes ??
        summaryData?.outcomes_recorded ??
        summaryData?.recorded_outcomes ??
    ...
    },
    {
      id: 4,
      value: `${String(
        summaryData?.outcome_summary?.adverse_outcome_rate ??
        outcomeData?.adverse_summary?.rate ??
        outcomeData?.adverse_outcome_rate ??
        0
      )}%`,
      label: 'Adverse Rate',
    },
            ...
            <View style={styles.headerContent}>
              <View style={styles.headerTop}>
                ...
                <View style={styles.headerTitleContainer}>
                  ...
                  <Text style={styles.headerSubtitle}>Delivery Point Reports</Text>
                </View>
