...
const RegisterPregnancyScreen = ({
  ...
  const handleNext = () => {
    if (currentStep === 1) {
      if (!formData.full_name || !formData.husband_name || !formData.age || !formData.mobile_number || !formData.address) {
  ...
  const formatReviewDate = (rawValue, displayValue = '') => {
    return displayValue || formatDateDDMMYYYY(rawValue);
  };
  ...
  const shouldShowMobileError = currentStep === 1 && formData.mobile_number.length > 0 && !isMobileValid;
  const shouldShowDuplicateMobileError = currentStep === 1 && isMobileValid && mobileDuplicateExists;
  const handleSubmit = async () => {
    ...
    try {
      ...
      const optionalFields = ['date_of_birth', 'abha_id', 'rch_id', 'hpr_id', 'blood_group', 'risk_factors'];
  ...
  const renderStep1 = () => <View>
      ...
      <View style={styles.formFields}>
        <View>
          ...
          <TextInput style={styles.input} placeholder={t('enterFullName')} value={formData.full_name} onChangeText={value => setFormData({
          ...formData,
          full_name: value
        })} />
        </View>
        ...
        <View>
          ...
          <TextInput style={styles.input} placeholder={t('enterHusbandName')} value={formData.husband_name} onChangeText={value => setFormData({
          ...formData,
          husband_name: value
        })} />
        </View>
        ...
        <View>
          ...
          color: '#6b7280'
        }]} placeholder="Auto-calculated from DOB" value={formData.age} editable={false} />
        </View>

        <View>
          <Text style={styles.label}>{t('mobileNumber')}  *</Text>
          <TextInput style={[styles.input, (shouldShowMobileError || shouldShowDuplicateMobileError) && styles.inputError]} placeholder={t('mobile10Digit')} keyboardType="phone-pad" maxLength={10} value={formData.mobile_number} onChangeText={value => setFormData({
          ...formData,
          mobile_number: value.replace(/[^0-9]/g, '')
        })} />
          {shouldShowMobileError && <Text style={styles.errorText}>{t('mobileNumberMust10Digits')}</Text>}
          {!shouldShowMobileError && isCheckingMobileDuplicate && <Text style={styles.helperText}>Checking mobile number...</Text>}
          {shouldShowDuplicateMobileError && <Text style={styles.errorText}>{t('duplicateMobileNumber')}</Text>}
        </View>

        <View>
          <Text style={styles.label}>{t('abhaId')}</Text>
          <TextInput style={styles.input} placeholder={t('enterABHAID')} value={formData.abha_id} onChangeText={value => setFormData({
          ...formData,
          abha_id: value
        })} />
        </View>
        ...
        <View>
          ...
          <TextInput style={styles.input} placeholder={t('enterRCHID')} value={formData.rch_id} onChangeText={value => setFormData({
          ...formData,
          rch_id: value
        })} />
        ...
        <View>
          ...
          <TextInput style={styles.input} placeholder={t('enterHPRID')} value={formData.hpr_id} onChangeText={value => setFormData({
          ...formData,
          hpr_id: value
        })} />
        </View>
        ...
        <View>
          ...
          <TouchableOpacity style={[styles.select, userInfo?.role === 'sub_centre' && styles.selectDisabled]} onPress={() => userInfo?.role !== 'sub_centre' && setShowBlockModal(true)} disabled={userInfo?.role === 'sub_centre'}>
          ...
          {userInfo?.role === 'sub_centre' && <Text style={styles.autoFilledText}>✓ Auto-filled from your profile</Text>}
        </View>

        <View>
          <Text style={styles.label}>{t('ward')} {wards.length > 0 ? '*' : ''}</Text>
          <TouchableOpacity style={styles.select} onPress={() => wards.length > 0 && setShowWardModal(true)} disabled={!formData.block_id || wards.length === 0}>
            <Text style={formData.ward_id ? styles.selectTextFilled : styles.selectText}>
              {wards.find(w => w.id === formData.ward_id)?.name || (formData.block_id ? wards.length > 0 ? t('selectWard') : 'No ward selection needed' : t('selectBlockFirst'))}
            </Text>
          </TouchableOpacity>
          {wards.length > 0 && !formData.ward_id && <Text style={styles.noteText}>* Ward selection is mandatory for this block</Text>}
        </View>

        <View>
          <Text style={styles.label}>{t('address')}  *</Text>
          <TextInput style={[styles.input, styles.textarea]} placeholder={t('enterFullAddress')} multiline numberOfLines={3} value={formData.address} onChangeText={value => setFormData({
          ...formData,
          address: value
        })} />
        </View>
  ...
  const renderStep2 = () => <View>
      ...
      <View style={styles.formFields}>
        <View>
          <Text style={styles.label}>{t('lastMenstrualPeriod')}  *</Text>
        ...
        <View>
          <Text style={styles.label}>{t('expectedDeliveryDateEDD')}  *</Text>
        ...
        <View style={styles.row}>
          <View style={styles.halfInput}>
            ...
            <TextInput style={styles.input} placeholder={t('noOfPregnancies')} keyboardType="numeric" value={formData.gravida} onChangeText={value => setFormData({
            ...formData,
            gravida: value.replace(/[^0-9]/g, '')
          })} />
          </View>
          <View style={styles.halfInput}>
            <Text style={styles.label}>{t('para')} </Text>
            <TextInput style={styles.input} placeholder={t('noOfLiveBirths')} keyboardType="numeric" value={formData.para} onChangeText={value => setFormData({
            ...formData,
            para: value.replace(/[^0-9]/g, '')
          })} />
          </View>
        </View>
        ...
        <View>
          ...
          <View style={styles.bloodGroupContainer}>
            {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(group => <TouchableOpacity key={group} style={[styles.bloodGroupButton, formData.blood_group === group && styles.bloodGroupSelected]} onPress={() => setFormData({
            ...formData,
            blood_group: group
          })}>
                <Text style={[styles.bloodGroupText, formData.blood_group === group && styles.bloodGroupTextSelected]}>
                  {group}
                </Text>
              </TouchableOpacity>)}
          </View>
          <TextInput style={[styles.input, {
          marginTop: 8
        }]} placeholder={t('orEnterBloodGroup')} value={formData.blood_group} onChangeText={value => setFormData({
          ...formData,
          blood_group: value
        })} />
        </View>
  ...
  const renderStep3 = () => <View>
      ...
      <View style={styles.reviewCard}>
        <View style={styles.reviewSection}>
          ...
          <ReviewItem label={t('name')} value={formData.full_name} />
          <ReviewItem label={t('husbandName')} value={formData.husband_name} />
          <ReviewItem label={t('age')} value={formData.age} />
          <ReviewItem label={t('dateOfBirth')} value={formatReviewDate(formData.date_of_birth, formData.date_of_birth_display)} />
          <ReviewItem label={t('mobile')} value={formData.mobile_number} />
          <ReviewItem label={t('abhaId')} value={formData.abha_id} />
          <ReviewItem label={t('rchId')} value={formData.rch_id} />
          <ReviewItem label={t('hprId')} value={formData.hpr_id} />
        </View>

        <View style={styles.reviewSection}>
          <Text style={styles.reviewSectionTitle}>{t('addressDetails')}</Text>
          <ReviewItem label={t('block')} value={blocks.find(b => b.id === formData.block_id)?.name || 'Not selected'} />
          <ReviewItem label={t('ward')} value={wards.find(w => w.id === formData.ward_id)?.name || 'Not selected'} />
          <ReviewItem label={t('address')} value={formData.address} />
        </View>

        <View style={styles.reviewSection}>
          <Text style={styles.reviewSectionTitle}>{t('pregnancyDetails')}</Text>
          <ReviewItem label={t('lastMenstrualPeriod')} value={formatReviewDate(formData.lmp_date, formData.lmp_date_display)} />
          <ReviewItem label={t('expectedDeliveryDateEDD')} value={formatReviewDate(formData.edd_date, formData.edd_date_display)} />
          <ReviewItem label={t('gravida')} value={formData.gravida || '0'} />
          <ReviewItem label={t('para')} value={formData.para || '0'} />
          <ReviewItem label={t('bloodGroup')} value={formData.blood_group} />
          <ReviewItem label={t('highRisk')} value={selectedRiskFactors.length > 0 ? 'Yes' : 'No'} />
          {selectedRiskFactors.length > 0 && <ReviewItem label={t('riskFactors')} value={selectedRiskFactors.join(', ')} />}
        </View>
  ...
  return <SafeAreaView style={styles.container} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <View style={styles.mainContent}>
          <View style={styles.card}>
            ...
            <View style={styles.navigationButtons}>
              {currentStep > 1 && <TouchableOpacity style={styles.backNavButton} onPress={handleBack}>
                  <Text style={styles.backNavButtonText}>{t('back')}</Text>
                </TouchableOpacity>}
              
              <TouchableOpacity style={[styles.nextNavButton, currentStep === 1 && styles.nextNavButtonFull, currentStep === 1 && (!isAgeValid || !isMobileValid || mobileDuplicateExists || isCheckingMobileDuplicate) && styles.nextNavButtonDisabled]} onPress={handleNext} disabled={submitting || currentStep === 1 && isCheckingMobileDuplicate}>
                {submitting ? <ActivityIndicator color="white" /> : <Text style={styles.nextNavButtonText}>
      ...
      {/* Block Modal */}
      <KeyboardSafeModal visible={showBlockModal} position="center" keyboardGap={15} overlayStyle={styles.centeredModalOverlay} contentStyle={styles.centeredModalContent} onRequestClose={() => {
      ...
      <KeyboardSafeModal visible={showWardModal} position="center" // This won't add keyboard padding
      onRequestClose={() => {
  ...
  <TextInput style={styles.searchInput} placeholder="Search ward..." value={wardSearch} onChangeText={setWardSearch} autoFocus={true} />
  <ScrollView style={{
          maxHeight: 400
  ...
  <TouchableOpacity style={styles.modalDoneButton} onPress={() => setShowRiskFactorsModal(false)}>
    <Text style={styles.modalDoneButtonText}>{t('done')}</Text>
      ...
      {/* Date Pickers */}
      {showDOBPicker && <DateTimePicker value={formData.date_of_birth ? new Date(formData.date_of_birth) : new Date()} mode="date" display={Platform.OS === 'ios' ? 'spinner' : 'default'} onChange={(event, date) => handleDateChange(event, date, 'dob')} maximumDate={new Date()} />}
      {showLMPPicker && <DateTimePicker value={formData.lmp_date ? new Date(formData.lmp_date) : new Date()} mode="date" display={Platform.OS === 'ios' ? 'spinner' : 'default'} onChange={(event, date) => handleDateChange(event, date, 'lmp')} maximumDate={new Date()} />}
      {showEDDPicker && <DateTimePicker value={formData.edd_date ? new Date(formData.edd_date) : new Date()} mode="date" display={Platform.OS === 'ios' ? 'spinner' : 'default'} onChange={(event, date) => handleDateChange(event, date, 'edd')} minimumDate={new Date()} />}
...
const styles = StyleSheet.create({
  ...
  centeredModalContent: {
    width: '90%',
    // Changed from '100%' to '90%' for better responsiveness
    maxWidth: 500,
    // Increased from 420 for larger screens
    maxHeight: '75%',
    minHeight: 320,
    backgroundColor: 'white',
    borderRadius: 20,
