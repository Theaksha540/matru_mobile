...
const resolveDateOfBirth = record => normalizeDateField(record?.date_of_birth || record?.dateOfBirth || record?.dob || record?.dob_date || '');
const BeneficiaryEditScreen = ({
  route,
  navigation
}) => {
  const {
  ...
  const currentDateOfBirth = resolveDateOfBirth(editedData) || resolveDateOfBirth(patient);
  const handleGoHome = () => {
    const roleRouteMap = {
      dp: 'DPDashboard',
  ...
  const loadPatientDetails = async () => {
    try {
      ...
      const normalizedDateOfBirth = resolveDateOfBirth(data);
      const normalizedPatient = data ? {
        ...data,
      ...
      setEditedData(normalizedPatient || {});
      const factors = normalizedPatient?.risk_factors ? normalizedPatient.risk_factors.split(',').map(f => f.trim()).filter(Boolean) : [];
      setSelectedRiskFactors(factors);

      // Load district name - wrapped in try-catch
      if (normalizedPatient?.district_id) {
        try {
          const districts = await syncService.getDistricts();
          const district = districts.find(d => d.id === normalizedPatient.district_id);
          setDistrictName(district?.name || normalizedPatient.district_id);
        } catch (error) {
          setDistrictName(normalizedPatient.district_id || 'N/A');
        }
      }

      // Load block name - wrapped in try-catch
      if (normalizedPatient?.block_id) {
        try {
          const blocks = await syncService.getBlocks();
          const block = blocks.find(b => b.id === normalizedPatient.block_id);
          setBlockName(block?.name || normalizedPatient.block_id);
        } catch (error) {
          setBlockName(normalizedPatient.block_id || 'N/A');
        }
      }

      // Load ward name - wrapped in try-catch
      if (normalizedPatient?.ward_id && normalizedPatient?.block_id) {
        try {
          const wards = await syncService.getWards(normalizedPatient.block_id);
          const ward = wards.find(w => w.id === normalizedPatient.ward_id);
          setWardName(ward?.name || normalizedPatient.ward_id);
        } catch (error) {
          setWardName(normalizedPatient.ward_id || 'N/A');
        }
      }

      // Load sub-centre name - wrapped in try-catch
      if (normalizedPatient?.sub_centre_id) {
        try {
          const subCentres = await adminAPI.getSubCentres();
          const subCentre = subCentres.find(s => s.id === normalizedPatient.sub_centre_id);
          setSubCentreName(subCentre?.name || normalizedPatient.sub_centre_id);
        } catch (error) {
          setSubCentreName(normalizedPatient.sub_centre_id || 'N/A');
        }
      }
    } catch (error) {
      Alert.alert(t('warning'), 'Some details could not be loaded, but you can still edit the beneficiary.', [{
        text: 'OK'
      }]);
    } finally {
      setLoading(false);
      ...
      <ScrollView style={styles.content}>
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.full_name || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            full_name: text
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.husband_name || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            husband_name: text
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.mobile_number || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            mobile_number: text.replace(/[^0-9]/g, '').slice(0, 10)
          }))} placeholder={t('enterMobileNumber')} keyboardType="numeric" maxLength={10} />
          </View>

          <View style={styles.infoRow}>
            <Text style={styles.label}>{t('age')}:</Text>
            <TextInput style={[styles.input, styles.readOnlyInput]} value={String(editedData?.age ?? '')} placeholder="Auto-calculated from DOB" editable={false} />
          </View>
            ...
            <View style={styles.datePickerContainer}>
              <TouchableOpacity style={styles.dateButton} onPress={() => setShowDobPicker(true)}>
                <Text style={styles.dateButtonText}>
                  {currentDateOfBirth ? formatDateDDMMYYYY(currentDateOfBirth) : t('selectDateDashFormat')}
                </Text>
                <Calendar size={16} color="#D2691E" />
              </TouchableOpacity>
              {showDobPicker && <DateTimePicker value={currentDateOfBirth ? new Date(currentDateOfBirth) : new Date()} mode="date" display={Platform.OS === 'ios' ? 'spinner' : 'default'} onChange={(event, date) => handleDateChange('date_of_birth', event, date)} maximumDate={new Date()} />}
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.address || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            address: text
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.abha_id || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            abha_id: text
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.rch_id || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            rch_id: text
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={editedData?.hpr_id || ''} onChangeText={text => setEditedData(prev => ({
            ...prev,
            hpr_id: text
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            <TextInput style={styles.input} value={String(editedData?.gravida ?? '')} onChangeText={text => setEditedData(prev => ({
            ...prev,
            gravida: text.replace(/[^0-9]/g, '').slice(0, 2)
          }))} placeholder={t('enterGravida')} keyboardType="numeric" />
          </View>

          <View style={styles.infoRow}>
            <Text style={styles.label}>{t('para')}:</Text>
            <TextInput style={styles.input} value={String(editedData?.para ?? '')} onChangeText={text => setEditedData(prev => ({
            ...prev,
            para: text.replace(/[^0-9]/g, '').slice(0, 2)
          }))} placeholder={t('enterPara')} keyboardType="numeric" />
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <TouchableOpacity style={styles.highRiskToggle} onPress={() => setEditedData(prev => ({
            ...prev,
            is_high_risk: !prev.is_high_risk
          }))}>
          ...
          {editedData?.is_high_risk && <View style={styles.infoRow}>
            ...
            flex: 1
                ...
                {selectedRiskFactors.length > 0 && <Text style={styles.selectedRiskText}>{selectedRiskFactors.join(', ')}</Text>}
              </View>
            </View>}
        </View>
