...
const PatientListScreen = ({ navigation, route }) => {
  ...
  const { mode, initialSearchQuery, action } = route?.params || {};
  ...
  const isWaitingForSync = (patient) =>
    !isOnline && Boolean(patient?.created_offline) && patient?.sync_status === 'pending';
  ...
  const handlePatientSelect = (patient) => {
    if (isCreateReferralMode) {
      ...
      if (!isPatientApproved(patient)) {
        Alert.alert(
          t('error'),
          'This beneficiary is not approved yet. Please approve first before scheduling USG appointment.'
        );
        return;
      }
      ...
      if (!isPatientApproved(patient)) {
        Alert.alert(
          t('error'),
          'This beneficiary is not approved yet. Please approve first before ANC tracking.'
        );
        return;
      }
      navigation.navigate('ANCTracking', { patientId: patient.id });
    }
  };
          ...
          <View style={styles.contentContainer}>
            {loading ? (
              ...
              <View style={styles.noDataContainer}>
                ...
                {action === 'createReferral' && !isOnline && (
                  <Text style={styles.noDataHint}>
                    {allPatients.length > 0 
                      ? 'All patients have active referrals (based on synced data)'
                      : 'No patient data synced. Please connect to internet and try again.'}
                  </Text>
                )}
                {action === 'createReferral' && isOnline && (
                  <Text style={styles.noDataHint}>
              ...
              <>
                {isCreateReferralMode && paginatedPatients.length > 0 && (
                  <View style={styles.infoBanner}>
                    <Text style={styles.infoBannerText}>
                      {isOnline 
                        ? `Showing only patients without active referrals (${filteredPatients.length} available)`
                        : `📵 Offline: Showing patients without active referrals based on synced data (${filteredPatients.length} available)`
                      }
                    </Text>
                  </View>
                )}
                    ...
                    <View style={styles.patientContent}>
                      ...
                      <View style={styles.patientInfo}>
                        <View style={styles.nameRow}>
                          ...
                          {patient.is_self_registered ? (
                            <View style={[styles.registrationBadge, styles.selfRegBadge]}>
                        ...
                        <View style={styles.approvalRow}>
                          ...
                          <View
                            style={[
                              ...
                              isWaitingForSync(patient)
                                ? styles.waitingSyncBadge
                                : isPatientApproved(patient)
