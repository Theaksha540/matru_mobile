...
const HighRiskCasesScreen = ({
  ...
  const loadHighRiskCases = async () => {
    try {
      ...
      const scopedWomen = isOnline ? await pregnantWomenAPI.getAll(scopedParams).then(response => Array.isArray(response?.items) ? response.items : Array.isArray(response) ? response : []) : await syncService.getPatients(scopedParams);
      const highRiskCases = scopedWomen.filter(woman => woman.is_high_risk || woman.age && (woman.age < 20 || woman.age > 35)).map(woman => ({
        id: woman.id,
  ...
  const filteredCases = cases.filter(caseItem => caseItem.name.toLowerCase().includes(searchQuery.toLowerCase()) || caseItem.phone.includes(searchQuery) || caseItem.rchId.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right', 'bottom']}>
        <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
          <View style={styles.header}>
            ...
            {fromNotification && initialSearchQuery && <View style={styles.notificationBanner}>
          ...
          <View style={styles.searchContainer}>
            <View style={styles.searchBox}>
              ...
              <TextInput style={styles.searchInput} placeholder="Search by name, phone, or RCH ID..." placeholderTextColor="#9ca3af" value={searchQuery} onChangeText={text => {
              setSearchQuery(text);
              setCurrentPage(1);
            }} />
            </View>
          ...
          <View style={styles.casesList}>
            {loading ? <View style={styles.loaderContainer}>
              ...
              </View> : paginatedCases.length === 0 ? <View style={styles.emptyStateContainer}>
                <Text style={styles.emptyStateText}>{t('noDataFound')}</Text>
              </View> : paginatedCases.map(caseItem => {
            const headerBg = caseItem.severity === 'high' ? '#D2691E' : '#FF8C00';
            return <LinearGradient key={caseItem.id} colors={['#fad5a5', '#e2e8f0']} start={{
              x: 0,
              y: 0
            }} end={{
              x: 1,
                ...
                backgroundColor: headerBg
                  ...
                  <View style={styles.caseDetails}>
                    ...
                    <View style={styles.riskBox}>
                      ...
                      {caseItem.riskFactors.map((factor, idx) => <Text key={idx} style={styles.riskItem}>• {factor}</Text>)}
                    </View>
          ...
          {filteredCases.length > itemsPerPage && <View style={styles.paginationContainer}>
              <TouchableOpacity style={[styles.paginationButton, currentPage === 1 && styles.paginationButtonDisabled]} onPress={handlePrevPage} disabled={currentPage === 1}>
              ...
              <TouchableOpacity style={[styles.paginationButton, currentPage === totalPages && styles.paginationButtonDisabled]} onPress={handleNextPage} disabled={currentPage === totalPages}>
        ...
        <Modal visible={showDetailModal} transparent animationType="slide" statusBarTranslucent onRequestClose={() => setShowDetailModal(false)}>
          <SafeAreaView style={styles.modalOverlay} edges={['top', 'left', 'right', 'bottom']}>
            <View style={[styles.modalContent, {
            marginLeft: Math.max(0, insets.left),
            marginRight: Math.max(0, insets.right),
            marginBottom: Math.max(8, insets.bottom)
          }]}>
              {/* Header */}
              <LinearGradient colors={['#D2691E', '#8B4513']} start={{
              x: 0,
              y: 0
            }} end={{
              x: 1,
                ...
                {selectedCase && <>
                    ...
                    <View style={styles.riskSection}>
                      ...
                      <View style={styles.riskList}>
                        {selectedCase.riskFactors.map((factor, idx) => <View key={idx} style={styles.riskItem}>
              ...
              <View style={styles.modalFooter}>
                <TouchableOpacity style={styles.closeModalButton} onPress={() => setShowDetailModal(false)} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  riskBox: {
    backgroundColor: 'rgba(139,101,73,0.1)',
    padding: 8,
    borderRadius: 6,
    marginVertical: 10,
    borderWidth: 1,
    borderColor: 'rgba(139,101,73,0.3)'
  },
  riskTitleRow: {
    flexDirection: 'row',
    alignItems: 'center',
