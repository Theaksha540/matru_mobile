...
const PendingApprovalDetailScreen = ({ route, navigation }) => {
  ...
  const loadPatientDetails = async () => {
    try {
      console.log('[PENDING-APPROVAL-DETAIL] loading patient details', {
        patientId,
        isOnline,
      });
      ...
      if (!data) {
        console.log('[PENDING-APPROVAL-DETAIL] No patient data found');
        Alert.alert(t('error'), t('failedToLoadPatientDetails'));
        setLoading(false);
        return;
      }
      
      console.log('[PENDING-APPROVAL-DETAIL] patient details loaded successfully', {
        patientId,
        loadedId: data?.id,
      ...
      console.error('[PENDING-APPROVAL-DETAIL] failed to load patient details', {
        patientId,
  ...
  const handleDateChange = (field, event, selectedDate) => {
    ...
    if (event.type === 'set' && selectedDate) {
      ...
      setEditedData({...editedData, [field]: formattedDate});
    }
  };

  const handleStartEdit = () => {
    setEditedData(patient || {});
    const factors = patient?.risk_factors
      ? patient.risk_factors.split(',').map(f => f.trim()).filter(f => f)
      : [];
    setSelectedRiskFactors(factors);
    setEditMode(true);
  };
    ...
    try {
      ...
      if (!isOnline) {
        if (shouldApprove) {
          Alert.alert(
            t('offlineMode'),
            'Approval is only available while online. Please connect to the internet and try again.'
          );
          return;
        }
  ...
  const getApprovalValidationMessage = (record) => {
    const { missing, invalid } = getMissingOrInvalidFields(record);
  ...
  const handleReject = () => {
    Alert.alert(
      t('rejectPatient'),
      t('areYouSureRejectRegistration'),
      [
        { text: t('cancel'), style: 'cancel' },
        { text: t('reject'), style: 'destructive', onPress: () => {
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              ...
              <Text style={styles.value}>{patient?.full_name || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, husband_name: text})}
                placeholder={t('enterHusbandName')}
              />
            ) : (
              <Text style={styles.value}>{patient?.husband_name || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, mobile_number: text.replace(/[^0-9]/g, '').slice(0, 10)})}
                placeholder={t('enterMobileNumber')}
                keyboardType="numeric"
                maxLength={10}
              />
            ) : (
              <Text style={styles.value}>{patient?.mobile_number || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              ...
              <Text style={styles.value}>{patient?.age || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <View style={styles.datePickerContainer}>
                ...
                {showDobPicker && (
                  <DateTimePicker
                    value={editedData?.date_of_birth ? new Date(editedData.date_of_birth) : new Date()}
                    mode="date"
              ...
              <Text style={styles.value}>{formatDateDDMMYYYY(patient?.date_of_birth)}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.label}>{t('aadhaarNumber')}:</Text>
            <Text style={styles.value}>{editMode ? (editedData?.aadhaar_masked || 'N/A') : (patient?.aadhaar_masked || 'N/A')}</Text>
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              ...
              <Text style={styles.value}>{patient?.address || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, abha_id: text})}
                placeholder={t('enterABHAID')}
              />
            ) : (
              <Text style={styles.value}>{patient?.abha_id || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, rch_id: text})}
                placeholder={t('enterRCHID')}
              />
            ) : (
              <Text style={styles.value}>{patient?.rch_id || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, hpr_id: text})}
                placeholder={t('enterHPRID')}
              />
            ) : (
              <Text style={styles.value}>{patient?.hpr_id || t('notAvailable')}</Text>
            )}
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              ...
              <Text style={styles.value}>{formatDateDDMMYYYY(patient?.lmp_date)}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              ...
              <Text style={styles.value}>{formatDateDDMMYYYY(patient?.edd_date)}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, gravida: text})}
                placeholder={t('enterGravida')}
                keyboardType="numeric"
              />
            ) : (
              <Text style={styles.value}>{patient?.gravida || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TextInput
                ...
                onChangeText={(text) => setEditedData({...editedData, para: text})}
                placeholder={t('enterPara')}
                keyboardType="numeric"
              />
            ) : (
              <Text style={styles.value}>{patient?.para || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              ...
              <Text style={styles.value}>{patient?.blood_group || t('notAvailable')}</Text>
            )}
          </View>
          <View style={styles.infoRow}>
            ...
            {editMode ? (
              <TouchableOpacity 
                ...
                onPress={() => setEditedData({...editedData, is_high_risk: !editedData.is_high_risk})}
              >
              ...
              <Text style={styles.value}>{patient?.is_high_risk ? t('yes') : t('no')}</Text>
            )}
          </View>
          {((editMode && editedData.is_high_risk) || (!editMode && patient?.is_high_risk)) && (
            <View style={styles.infoRow}>
              ...
              {editMode ? (
                ...
                <Text style={styles.value}>{patient?.risk_factors || t('notAvailable')}</Text>
              )}
            </View>
          )}
        </View>
        ...
        <ScrollView style={{ maxHeight: 400 }} keyboardShouldPersistTaps="handled">
          {bloodGroupOptions.map(group => (
            <TouchableOpacity
              ...
              style={[
                styles.modalOption,
                editedData?.blood_group === group && styles.modalOptionSelected
              ]}
              onPress={() => {
                setEditedData({ ...editedData, blood_group: group });
                setShowBloodGroupModal(false);
              }}
            >
              <Text style={[
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', padding: 16, backgroundColor: '#D2691E', gap: 12 },
  backButton: {
    width: 36,
    height: 36,
    borderRadius: 18,
  ...
  label: { fontSize: 14, color: '#6b7280', width: 120 },
  value: { fontSize: 14, color: '#1f2937', flex: 1, fontWeight: '500' },
  actionButtons: { flexDirection: 'row', gap: 12, marginTop: 8, marginBottom: 24 },
  approveButton: { flex: 1, backgroundColor: '#16a34a', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 14, borderRadius: 8, gap: 8 },
  rejectButton: { flex: 1, backgroundColor: '#dc2626', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 14, borderRadius: 8, gap: 8 },
  buttonText: { color: 'white', fontSize: 16, fontWeight: '600' },
  cancelButton: { flex: 1, backgroundColor: '#6b7280', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 14, borderRadius: 8, gap: 8 },
  input: { flex: 1, borderWidth: 1, borderColor: '#d1d5db', borderRadius: 6, padding: 8, fontSize: 14, color: '#1f2937' },
  readOnlyInput: { backgroundColor: '#f9fafb', color: '#6b7280' },
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fad5a5' },
  loadingText: { marginTop: 12, fontSize: 16, color: '#6b7280', fontWeight: '500' },
  datePickerContainer: { flex: 1 },
  dateButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: '#d1d5db', borderRadius: 6, padding: 8, minHeight: 36 },
  dateButtonText: { fontSize: 14, color: '#1f2937', flex: 1, marginRight: 8 },
  riskFactorsButton: { padding: 12, borderWidth: 1, borderColor: '#d1d5db', borderRadius: 6, backgroundColor: 'white', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  riskFactorsButtonText: { fontSize: 14, color: '#1f2937' },
  selectedRiskText: { fontSize: 12, color: '#10B981', marginTop: 4, fontStyle: 'italic' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#e5e7eb' },
  modalFooter: { paddingHorizontal: 16, paddingTop: 8, paddingBottom: 16, backgroundColor: 'white', borderTopWidth: 1, borderTopColor: '#f3f4f6' },
  ...
  modalOption: { padding: 16, borderBottomWidth: 1, borderBottomColor: '#f3f4f6', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  modalDoneButton: { minHeight: 48, paddingHorizontal: 16, paddingVertical: 12, backgroundColor: '#D2691E', alignItems: 'center', justifyContent: 'center', borderRadius: 10, width: '40%', alignSelf: 'center' },
  modalDoneButtonText: { fontSize: 18, fontWeight: '600', color: 'white' },
  highRiskToggle: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8 },
  toggleSwitch: { width: 44, height: 24, borderRadius: 12, backgroundColor: '#d1d5db', padding: 2, justifyContent: 'center' },
  toggleSwitchActive: { backgroundColor: '#10B981', alignItems: 'flex-end' },
