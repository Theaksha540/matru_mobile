...
const PendingApprovalScreen = ({ navigation }) => {
  ...
  const loadPendingApprovals = async () => {
    try {
      const [data, userInfo, offlineApprovalsData] = await Promise.all([
        syncService.getPendingApprovals(),
        secureStorage.getItem('user_info'),
        syncService.getOfflineApprovals(),
      ]);

      console.log('[PENDING-APPROVAL] Loaded data:', {
        pendingCount: Array.isArray(data) ? data.length : 0,
        offlineApprovalsCount: offlineApprovalsData?.length || 0,
        isOnline,
      });
      ...
      console.log('[PENDING-APPROVAL] Filtered out offline approvals:', offlineApprovalIds.length);
      
      const scopedList = userInfo?.role === 'sub_centre' && userInfo?.block_id
        ? filteredList.filter((item) => String(item?.block_id) === String(userInfo.block_id))
        : filteredList;

      console.log('[PENDING-APPROVAL] Final list count:', scopedList.length);
  ...
  const renderPendingItem = (item) => {
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate('PendingApprovalDetail', { patientId: item.id })}
        activeOpacity={0.7}
      >
        <View style={styles.itemHeader}>
          ...
          <View style={styles.badgeContainer}>
            {isOfflineApproved ? (
              <View style={styles.offlineApprovedBadge}>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchInputWrapper}>
          ...
          <TextInput
            ...
            placeholder="Search by name, mobile, or age"
            value={searchQuery}
